...
try {
  ...
  $sql = $conexion->prepare("SELECT m.id AS manifiesto_id, oc.cli_id AS cliente_id FROM cmx_manifiesto m
  ...
  WHERE m.estado_seguimiento = 'SEGUIMIENTO' AND ir.num_manifiesto IS NOT NULL /* AND oc.cli_id=455 */
  ...
  foreach ($datos_clientes as $value) {
    ...
    // Agregar el manifiesto al cliente
  ...
  if (!empty($agrupados_por_cliente)) {
    foreach ($agrupados_por_cliente as $cliente_id => &$data) {
      $sql = $conexion->prepare("SELECT g.id_cliente, g.nombre_grupo, gc.nombre_contactos, gc.email, /* ch.hora_envio, */ gc.idgrupo, gc.id AS contacto_id, n.novedad,n.id AS novedad_id
          FROM cmx_grupo g
        ...
        foreach ($data['contactos'] as $contactos_grupo) {
          foreach ($contactos_grupo as $contacto) {
            foreach ($contacto['novedades'] as $novedad) {
              ...
              /*INNER JOIN (SELECT cod_ini_ruta, MAX(CONCAT(fecha, ' ', hora)) AS max_fecha_hora FROM cmx_inicio_seguimiento WHERE novedad = :novedad GROUP BY cod_ini_ruta) AS ult_novedad  ON ise.cod_ini_ruta = ult_novedad.cod_ini_ruta AND 
              CONCAT(ise.fecha, ' ', ise.hora) = ult_novedad.max_fecha_hora*/
        ...
        foreach ($datos_horas_envio as  $contacto) {
          ...
          if ($hora_contacto >= $horaActual && $hora_contacto < $horaActual + 1) {
            ...
            if (!empty($agrupadas)) {
              ...
              // $mail->Subject = "Notificación de Solicitudes";
              ...
              $mail->AddEmbeddedImage('/var/www/html/principal/' . 'public/img/magnetron.png', 'magnetron_logo', 'magnetron.png');
              $mail->AddEmbeddedImage('/var/www/html/principal/' . 'public/img/nexos.png', 'nexos_logo', 'nexos.png');

              $template = file_get_contents('/var/www/html/principal/views/templates/magnetron.php');
              $contenidoDinamico = '';
              ...
              foreach ($agrupadas as $solicitud => $datosAgrupados) {
                foreach ($datosAgrupados as $dato) {
                  ...
                  // $sqlm = $conexion->prepare("INSERT INTO cmx_mail_enviados (id, id_servicio, manifesto_id, id_fecha_parametro, fecha, hora, usuario, modalidad) VALUES (null, :nundoc_solicitud, :manifiesto, :seguimiento_inicio_id, :fecha, :hora, :usuario, 'Automático')");
                  ...
                  // $sqlm->bindParam(':fecha', date('Y-m-d'));
                  // $sqlm->bindParam(':hora', date('H:i:s'));
                  // $sqlm->bindParam(':usuario', 'system'); // Asegúrate de definir $usuario previamente
                  // $sqlm->execute();
                }
              }
              ...
              if ($procesado == true) {
                ...
                if ($mail->Send()) {
                  echo "Correo agrupado enviado con éxito" . "\n" . "<br/>";
                } else {
            ...
            if (!empty($no_agrupadas)) {
              ...
              echo "No hay datos no agrupados" . "<br/>";
            }
          } else {
            echo "NO HAY HORAS DE ENVIO PARA EL CORRO";
          }
        }
      }
    }
