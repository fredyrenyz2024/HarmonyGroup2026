...
class web_serviceModel extends Model
{
	...
	public function Genera_Consulta_Manifiesto($filtro, $num_docu)
	{
		...
		try {
			$sql = "SELECT *
					 FROM cmx_xml_manifiesto 
					 WHERE num_manifiesto=" . $num_docu;
			$resultado = $this->_db3->query($sql);
			$resultado->setFetchMode(PDO::FETCH_ASSOC);
			return $resultado->fetchall();
		} catch (PDOException $e) {
	...
	public function Vehiculo_Ministerio($placa)
	{
		try {
			...
			$sql = "SELECT ve.placa, ma.rndc_id AS rndc_marca, lin.rndc_id AS rndc_linea,
					...
					gu.rndc_id AS rndc_vehiculo, gu.rndc_id_trailer AS rndc_trailer_id, ve2.peso,
					...
					INNER JOIN cmx_rndc_vehiculos_configuracion gu ON ve2.configuracion=gu.id
					INNER JOIN cmx_rndc_vehiculos_carroceria carro ON ve.tipo_carroceria=carro.id
	...
	public function Transaccion_Nexos_Min2($documento, $xml, $respuesta, $estadoterg, $accion, $tipo)
	{
		try {
			...
			$resultado->prepare("insert into web_service_RNDC(id,codigo_proceso,tipo,estado_envio_rndc,estado,cadena_xml,fecha,hora,usuario,rta_ministerio,tipo_tercero,accion)values(:id,:proceso,:tipo,:estado_envio,:estado,:cadena,:fecha,:hora,:user,:respuesta,:tipot,:accion)")->execute(
				array(
					':id' => null,
					':proceso' => $documento,
					':tipo' => $tipo,
	...
	public function Datos_Remesa($id_remesa)
	{
		try {
			$sql = "SELECT re.id,mr.id_manifiesto, me.naturaleza,
					...
					ON re.id=mr.id_remesa AND mr.estado=1
					...
					LEFT JOIN cmx_remesa_ordencargue remo
					ON remo.id_remesa=re.id
					LEFT JOIN cmx_orden_cargue oc
					ON remo.id_orden_cargue=oc.id
					...
					LEFT JOIN cmx_cotizaciones_serviciocliente coti
					ON se.n_cotizacion=coti.n_cotizacion
					LEFT JOIN cmx_clientes cli
					ON coti.nombre_cliente=cli.nombre
	...
	public function Consulta_Manifiesto($manifiesto)
	{
		try {
			$sql = "SELECT ma.id, ma.fecha_expedicion,
					...
					ON ma.conductor_manifiesto=cond.numero_documento
					INNER JOIN cmx_vehiculos ve
					ON ma.placa=ve.placa 
					LEFT JOIN cmx_trailer_vehiculo tve
					ON ve.id=tve.id_vehiculo 
					AND tve.estado=1
					LEFT JOIN cmx_trailer tra
					ON tve.id_trailer=tra.id
					LEFT JOIN cmx_manifiesto_anticipo man
					ON ma.id=man.id_manifiesto
					WHERE ma.id=" . $manifiesto;
			$resultado = $this->_db3->query($sql);
			$resultado->setFetchMode(PDO::FETCH_ASSOC);
			return $resultado->fetchall();
