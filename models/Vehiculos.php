...
class Vehiculos
{
	...
	public function crearVehiculo()
	{
		...
		if ($result) {
			...
			$sql2 = "
			    	...
			    	'$f_matricula',
			    	'$clase')";
			$crearVehiculod = $conexion->prepare($sql2);
			$result2 = $crearVehiculod->execute();
			//ingresar datos en tabla cm_detalle_vehiculo
			$sql3 = "INSERT INTO cmx_detalle_vehiculo
							(id,
							tecnomecanica,
							...
							'$id_vehiculo',
							'$lice_transito')";
			$creardetalleVehiculo = $conexion->prepare($sql3);
			$result3 = $creardetalleVehiculo->execute();
			//FOTO LICENCIA TRANSITO
			$ruta_tran = "../public/files/vehiculos/" . $id_vehiculo . "/TRA/";
			$ruta_btran = "public/files/vehiculos/" . $id_vehiculo . "/TRA/";
			$namet = $_POST['name_transi'];
			$sqlm = "UPDATE cmx_detalle_vehiculo
							SET foto_transito='$ruta_btran',
								name_transito='$namet'
							WHERE id_vehiculo =" . $id_vehiculo . "";
			$consulta_transi = $conexion->prepare($sqlm);
			$consulta_transi->execute();
			if (!file_exists($ruta_tran)) {
				mkdir($ruta_tran, 0777, true);
			}
			for ($k = 0; $k < count($_FILES); $k++) {
				if (isset($_FILES["foto_transi" . $k])) {
					$file = $_FILES["foto_transi" . $k];
					$nombre = $file["name"];
					$tipo = $file["type"];
			...
			if ($_POST["trailers"] != '') {
				$sqlt = "INSERT INTO cmx_trailer_vehiculo(id,id_vehiculo,id_trailer,estado,fecha,hora,usuario)
							VALUES(NULL,'$id_vehiculo','$placa_trailer','1','$fecha','$hora','$id_usuario')";
				$crear_trailer = $conexion->prepare($sqlt);
				$result_trailer = $crear_trailer->execute();
				$sqlut = "
							UPDATE cmx_trailer 
			...
			$result_log = $crear_oper_vehi->execute();
		...
		//VALIDAR RESULTADOS DE TRANSACCIONES
		if ($result && $result2 && $result3 && $result_log) {
			$sql_transaccion = "INSERT INTO web_service_RNDC(id,codigo_proceso,tipo,estado_envio_rndc,estado,fecha,hora,usuario,accion)
						VALUES(null,'" . $placa . "','Vehiculo',0,1,'" . $fecha . "','" . $hora . "','" . $nom_usuario . "','Crear')";
			$crear_transaccion = $conexion->prepare($sql_transaccion);
			$crear_transaccion->execute();
			if ($_POST["trailers"] == '' && $crear_transaccion) {
				$return["success"] = true;
				$_msg_error = "";
				$return["error"] = $_msg_error;
			}
			if ($_POST["trailers"] != '' && $result_trailer && $crear_transaccion) {
				$return["success"] = true;
				$_msg_error = "";
				$return["error"] = $_msg_error;
			} else {
	...
	public function verVehiculo()
	{
		...
		// 			tra.placa AS 'placa_trailer',emp.operador_gps,
		// 			mak.marca AS marcav, line.descripcion AS lineav, col.color AS colorv, ase.nombre AS asegv, chas.descripcion AS carrov,
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ) > 0,
		// 				( SELECT cp1.rndc_id FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ), 
		// 				NULL
		// 			) 'rndc_propietario',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ) > 0,
		// 				( SELECT cp1.tipo_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ), 
		// 				NULL
		...
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ) > 0,
		// 				( SELECT cp1.digito_verificacion FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ), 
		// 				NULL
		// 			) 'digito_verificacion_propietario',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ) > 0,
		// 				( SELECT cp1.nombre FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario ), 
		// 				NULL
		// 			) 'nombre_propietario',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ) > 0,
		// 				( SELECT cp1.rndc_id FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ), 
		// 				NULL
		// 			) 'rndc_tenedor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ) > 0,
		// 				( SELECT cp1.tipo_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ), 
		// 				NULL
		...
		// 			) 'documento_tenedor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ) > 0,
		// 				( SELECT cp1.digito_verificacion FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ), 
		// 				NULL
		// 			) 'digito_verificacion_tenedor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ) > 0,
		// 				( SELECT cp1.nombre FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor ), 
		// 				NULL
		// 			) 'nombre_tenedor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ) > 0,
		// 				( SELECT cp1.rndc_id FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ), 
		// 				NULL
		// 			) 'rndc_conductor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ) > 0,
		// 				( SELECT cp1.tipo_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ), 
		// 				NULL
		...
		// 			) 'documento_conductor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ) > 0,
		// 				( SELECT cp1.digito_verificacion FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ), 
		// 				NULL
		// 			) 'digito_verificacion_conductor',
		// 			IF( ( SELECT COUNT(cp1.id) FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ) > 0,
		// 				( SELECT cp1.nombre FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor ), 
		// 				NULL
		// 			) 'nombre_conductor'
		// 		FROM
		// 			cmx_vehiculos cv
		...
		// 			ON dv.marca=mak.id
		// 			LEFT JOIN cmx_rndc_vehiculos_linea line
		// 			ON dv.linea=line.id
		// 			LEFT JOIN cmx_rndc_vehiculos_color col
		// 			ON dv.color=col.id
		// 			LEFT JOIN cmx_rndc_aseguradoras ase
		// 			ON dv.aseguradora=ase.id
		// 			LEFT JOIN cmx_rndc_vehiculos_carroceria chas
		// 			ON cv.tipo_carroceria=chas.id	
		// 			LEFT JOIN cmx_detalle_vehiculo cdv
		// 			ON cv.id=cdv.id_vehiculo
		// 			LEFT JOIN cmx_rndc_empresa_gps	emp
		// 			ON cv.empresa_gps=emp.id
		// 			LEFT JOIN cmx_trailer_vehiculo trav
		// 			ON cv.id=trav.id_vehiculo
		// 			LEFT JOIN  cmx_trailer tra
		// 			ON trav.id_trailer=tra.id
		// 		WHERE cv.id = $id_vehiculo GROUP BY cv.id";

		$sql = "SELECT 
			cla.clase,
			dv.*,
			cf.nombre AS sigla_c, 
			cf.descripcion AS des_c,
			cv.*,
			cdv.*,
			tra.placa AS 'placa_trailer',
			emp.operador_gps,
			mak.marca AS marcav, 
			line.descripcion AS lineav, 
			col.color AS colorv, 
			ase.nombre AS asegv, 
			chas.descripcion AS carrov,
			-- Información del propietario
			(SELECT cp1.rndc_id FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario) AS rndc_propietario,
			(SELECT cp1.tipo_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario) AS tipo_documento_propietario,
			(SELECT cp1.numero_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario) AS documento_propietario,
			(SELECT cp1.digito_verificacion FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario) AS digito_verificacion_propietario,
			(SELECT CASE WHEN cp1.tipo_documento = 'NIT' THEN cp1.nombre WHEN cp1.tipo_documento = 'Cedula de Ciudadania' THEN CONCAT(cp1.nombre, ' ', cp1.apellido1, ' ', cp1.apellido2) ELSE NULL END
				FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_propietario) AS nombre_propietario,
			-- Información del tenedor
			(SELECT cp1.rndc_id FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor) AS rndc_tenedor,
			(SELECT cp1.tipo_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor) AS tipo_documento_tenedor,
			(SELECT cp1.numero_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor) AS documento_tenedor,
			(SELECT cp1.digito_verificacion FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor) AS digito_verificacion_tenedor,
			(SELECT CASE WHEN cp1.tipo_documento = 'NIT' THEN cp1.nombre WHEN cp1.tipo_documento = 'Cedula de Ciudadania' THEN CONCAT(cp1.nombre, ' ', cp1.apellido1, ' ', cp1.apellido2) ELSE NULL END
				FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_tenedor) AS nombre_tenedor,
			-- Información del conductor
			(SELECT cp1.rndc_id FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor) AS rndc_conductor,
			(SELECT cp1.tipo_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor) AS tipo_documento_conductor,
			(SELECT cp1.numero_documento FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor) AS documento_conductor,
			(SELECT cp1.digito_verificacion FROM cmx_proveedores cp1 WHERE cp1.numdoc_nexos = cv.id_conductor) AS digito_verificacion_conductor,
			(SELECT CONCAT(cp1.nombre, ' ', cp1.apellido1, ' ', cp1.apellido2) FROM cmx_proveedores cp1 
			WHERE cp1.numdoc_nexos = cv.id_conductor) AS nombre_conductor
			FROM 
			cmx_vehiculos cv
			LEFT JOIN cmx_vehiculo2 dv ON cv.numdoc_vehiculo = dv.id_vehiculo
			INNER JOIN cmx_rndc_vehiculos_configuracion cf ON dv.configuracion = cf.id
			LEFT JOIN cmx_rndc_clase_vehiculo cla ON dv.clase_vehiculo = cla.id
			LEFT JOIN cmx_rndc_vehiculos_marcas mak ON dv.marca = mak.id
			LEFT JOIN cmx_rndc_vehiculos_linea line ON dv.linea = line.id
			LEFT JOIN cmx_rndc_vehiculos_color col ON dv.color = col.id
			LEFT JOIN cmx_rndc_aseguradoras ase ON dv.aseguradora = ase.id
			LEFT JOIN cmx_rndc_vehiculos_carroceria chas ON cv.tipo_carroceria = chas.id
			LEFT JOIN cmx_detalle_vehiculo cdv ON cv.numdoc_vehiculo = cdv.id_vehiculo
			LEFT JOIN cmx_rndc_empresa_gps emp ON cv.empresa_gps = emp.id
			LEFT JOIN cmx_trailer_vehiculo trav ON cv.numdoc_vehiculo = trav.id_vehiculo
			LEFT JOIN cmx_trailer tra ON trav.id_trailer = tra.id
			WHERE cv.numdoc_vehiculo=$id_vehiculo GROUP BY cv.numdoc_vehiculo";
	...
	public function editarvehiculonew()
	{
		...
		$e_ntransito = $_POST["e_ntransito"];
		$ecant_viaje = $_POST["ecant_viaje"];

		//fotos del vehiculo
		if (isset($_POST["newtransi"])) {
			$newtransi = $_POST["newtransi"];
		}
		...
		$foto_li1 = "../public/files/vehiculos/" . $id_vehiculo . "/TRA/";
		$foto_li2 = "public/files/vehiculos/" . $id_vehiculo . "/TRA/";
		...
		if (file_exists($foto_li1)) {
			if (file_exists($foto_li1)	&& $newtransi != '') {
				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newtransi;
				$sql = "UPDATE cmx_detalle_vehiculo
						SET 
						foto_transito='public/files/vehiculos/" . $id_vehiculo . "/TRA/',
						name_transito='" . $nom2 . "'
						WHERE id_vehiculo=" . $id_vehiculo . " ";
				...
				//mover
				for ($k = 0; $k < count($_FILES); $k++) {
					if (isset($_FILES["e_foto_licen" . $k])) {
						$file = $_FILES["e_foto_licen" . $k];
		...
		if (file_exists($foto_caro1)) {
			if (file_exists($foto_caro1)	&& $newavf != '') {
				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newavf;
				$sql = "UPDATE cmx_detalle_vehiculo
						SET 
						foto_vehiculo='public/files/vehiculos/" . $id_vehiculo . "/F/',
						name_frontal='" . $nom2 . "'
						WHERE id_vehiculo=" . $id_vehiculo . " ";
		...
		if (file_exists($foto_dere1)) {
			if (file_exists($foto_dere1) && $newavd != '') {
				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newavd;

				$sqld = "UPDATE cmx_detalle_vehiculo
					SET 
					foto_derecha='public/files/vehiculos/" . $id_vehiculo . "/D/',
					name_derecha='" . $nom2 . "'
					WHERE id_vehiculo=" . $id_vehiculo . " ";
		...
		if (file_exists($foto_iz1)) {
			if (file_exists($foto_iz1) && $newavi != '') {
				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newavi;

				$sql = "UPDATE cmx_detalle_vehiculo
					SET 
					foto_izquierda='public/files/vehiculos/" . $id_vehiculo . "/I/',
					name_izquierda='" . $nom2 . "'
					WHERE id_vehiculo=" . $id_vehiculo . " ";
		...
		if (file_exists($foto_at1)) {
			if (file_exists($foto_at1) &&  $newava != '') {
				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newava;

				$sql = "UPDATE cmx_detalle_vehiculo
					SET 
					foto_atras='public/files/vehiculos/" . $id_vehiculo . "/A/',
					name_atras='" . $nom2 . "'
					WHERE id_vehiculo=" . $id_vehiculo . " ";
		...
		if (file_exists($foto_soat1)) {
			if (file_exists($foto_soat1) && $newso != '') {
				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newso;

				$sql = "UPDATE cmx_detalle_vehiculo
					SET 
					foto_soat='public/files/vehiculos/" . $id_vehiculo . "/SO/',
					name_soat='" . $nom2 . "'
					WHERE id_vehiculo=" . $id_vehiculo . " ";
		...
		if (file_exists($foto_tecno1)) {
			if (file_exists($foto_tecno1) && $newte != '') {

				$aleatorio1 = rand(10000, 90000);
				$aleatorio2 = rand(10000, 90000);
				$nom2 = $aleatorio1 . $newte;

				$sql = "UPDATE cmx_detalle_vehiculo
					SET 
					foto_tecno='public/files/vehiculos/" . $id_vehiculo . "/TECNO/',
					name_tecno='" . $nom2 . "'
					WHERE id_vehiculo=" . $id_vehiculo . " ";
		...
		if (isset($placat)) {
			...
			if ($total == 0) {
				...
				if ($result_z) {
					...
					$sql10 = "UPDATE cmx_trailer_vehiculo
								 SET estado=0
								 WHERE id_trailer=" . $trailer_anterior . "  ";
		...
		if ($result) {
			...
			$sqlh = "INSERT INTO cmx_asignar_conductorvehiculo (id,cod_vehiculo,cod_conductor,estado,fecha_actual) VALUES(null,'" . $placa . "','" . $docconductor . "','1','" . $fecha . "')";
		...
		$sql_transaccion = "INSERT INTO web_service_RNDC (id,codigo_proceso,tipo,estado_envio_rndc,estado,fecha,hora,usuario) VALUES(null,'" . $id_vehiculo . "','Vehiculo',0,1,'" . $fecha . "','" . $hora . "','" . $idusuario . "')";
	...
	public function cargarpropietario()
	{
		$sql = "
	            ...
	            WHERE 
	                cp.numdoc_nexos= cap.id_proveedor
