<?php
class unidades_despachoModel extends Model
{
	...
	public function getUnidades_despacho()
	{
		$unidades_despacho = $this->_db->getConsulta("SELECT * FROM cmx_unidad_despacho");
		return $unidades_despacho;
	}

	public function getTabla()
	{
		$unidades_despacho = $this->_db3->prepare("SELECT * FROM cmx_unidad_despacho");
		$unidades_despacho->execute();
		$unidades_despacho = $unidades_despacho->fetchAll(PDO::FETCH_ASSOC);
		return $unidades_despacho;
	}
	...
	public function getHtmlSelect($name, $id)
	{
		...
		if ($name) {
			$query = 'SELECT * FROM cmx_unidad_despacho cud';
			...
			if ($array) {
				// print_r($array);
				$select = '
						<select class="select2 select2-hidden-accessible" name="' . $name . '" id="slct_unidad_despacho_' . $id . '" aria-hidden="true">
					';
	...
	public function getHtmlSelectMultiple($name, $id)
	{
		...
		if ($name) {
			...
			if ($array) {
				// print_r($array);
				$select = '
						<select class="tags" multiple="multiple" name="' . $name . '" id="slct_unidad_despacho_' . $id . '" aria-hidden="true">
					';
				foreach ($array['rowsData'] as $key => $value) {
	...
	{
		$sql = $this->_db3->prepare("SHOW COLUMNS FROM 
					cmx_unidad_despacho 
				LIKE 'tipo_unidad_despacho'");
		$sql->execute();
		...
		$select = '<select class="select2 select2-hidden-accessible" name="' . $name . '" id="slct_tipo_unidad_despacho_' . $id . '" aria-hidden="true">';
