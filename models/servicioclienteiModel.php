...
class servicioclienteiModel extends Model
{
            ...
            if ($estado == "Todas") {
                try {
                    ...
                    if ($_SESSION['usuario']['id_perfil'] === 7) {
                        ...
                        INNER JOIN cmx_detalle_mercancia2 m ON ss.idpareja_origen_destino = m.id
                        ...
                        $query .= " GROUP BY ss.nundoc_solicitud ORDER BY coti.id DESC";
                        ...
                        $query = "SELECT ss.nundoc_solicitud, coti.nombre_cliente, m.itr, coti.estado AS estado_autorizacion, 
                            m.tipo_servicio_mer, m.peso_neto_kg, CONCAT(ss.fecha, '-', ss.hora) AS fecha_solicitud_servicio,
                            ss.estado, coti.id, coti.n_cotizacion, m.tipo_transporte, m.tipo_mercancia, coti.estado_autorizado,
                            ss.prioritaria, emp.nombre_empresa,cl.id AS Cliente_Id, m.n_cotizacion,
                            CASE 
                                WHEN ec.estado IS NOT NULL THEN ec.estado 
                                WHEN ses.estado IS NOT NULL THEN ses.estado 
                                ELSE 'Sin Estado'
                ...
                try {
                    ...
                    $sqlBase = "SELECT ss.nundoc_solicitud, coti.nombre_cliente, m.itr, coti.estado AS estado_autorizacion, 
                            ...
                            FROM cmx_cotizaciones_serviciocliente coti
                                ...
                                LEFT JOIN cmx_estudiov_completo ec ON se.id_solicitudpreestudio = ec.id_estudio AND ec.estado_actu = 1
                                LEFT JOIN cmx_solicitudes_estados ses ON se.id_solicitudpreestudio = ses.id_solicitud AND ses.estado_actual = 1
                                LEFT JOIN cmx_detalle_prioridad dp ON ss.nundoc_solicitud=dp.numdoc_solicitud
                                WHERE coti.fecha_creacion >= '2025-01-01'
                    ...
                    $sqlBase .= " GROUP BY ss.nundoc_solicitud ORDER BY coti.id DESC";
    ...
    public function Visualizar_Mercancias($numero_cotizacion)
    {
        ...
        C1.rndc_codigo_ciudad AS Codigo_Origen, C2.rndc_codigo_ciudad AS Codigo_Destino
    					FROM cmx_cotizaciones_serviciocliente coti
                        INNER JOIN cmx_solicitud_vehiculo2 ss ON coti.n_cotizacion = ss.n_cotizacion
                        INNER JOIN cmx_detalle_mercancia2 cdm ON ss.idpareja_origen_destino = cdm.id
    					INNER JOIN cmx_para_tipo_vehiculo tv ON cdm.tipo_vehiculo=tv.id
    					INNER JOIN cmx_municipios C1 ON cdm.origen=C1.rndc_codigo_ciudad
    					INNER JOIN cmx_municipios C2 ON cdm.destino=C2.rndc_codigo_ciudad
    					INNER JOIN cmx_para_tipo_empaque e ON cdm.tipo_empaque=e.id
    					WHERE ss.nundoc_solicitud=" . $numero_cotizacion . "");
    ...
    private function insertarDestinatarios($destinatarios, $bloqueActual, $num_solicitud)
    {
        ...
        $sql = "
            ...
            fecha_estimada_entrega, observacion, fecha, hora, usuario,
            hora_estimada, tipo, orden, telefono, peso, lugar, id_punto, estado_destinatario, valor_tarifa)
            VALUES
            (NULL, :solicitud, :ciudad, :direccion, :cliente,
            :fecha_estimada, :observacion, :fecha, :hora, :user,
            :hora_estimada, 'punto entrega', :orden, :telefono, :peso, :lugar, :id_punto, 'PENDIENTE', :valor_tarifa)
        ";
        ...
        foreach ($destinatarios as $d) {
            ...
            $stmt->execute([
                ...
                ':lugar'          => $d['lugar'],
                ':id_punto'       => $id_punto,
                // ':valor_tarifa'   => $d['valor_tarifa_venta']
                ':valor_tarifa' => str_replace(',', '', $d['valor_tarifa_venta'])
            ]);
    ...
    {
        $response = [];
        $usuario_aprueba = $_SESSION["usuario"]["nom_usuario"];
        ...
        try {
            ...
            UPDATE cmx_solicitud_vehiculo2 
            SET prioritaria = :prioritaria 
        ...
        ");
            ...
            // 2️⃣ Actualizar detalle de prioridad (solo si se aprueba)
            if ($estado === 'Aprobada') {

                $sqlDetalle = $this->_db3->prepare("
                UPDATE cmx_detalle_prioridad
                SET 
                    usuario_aprueba = :usuario_aprueba,
                    fecha_aprueba   = CURDATE(),
                    hora_aprueba    = CURTIME()
                WHERE numdoc_solicitud = :numdoc_solicitud
            ");

                $sqlDetalle->bindParam(':usuario_aprueba', $usuario_aprueba, PDO::PARAM_STR);
                $sqlDetalle->bindParam(':numdoc_solicitud', $numdoc_solicitud, PDO::PARAM_STR);
                $sqlDetalle->execute();
            }

            return [
                'status' => 200,
                'message' => 'Solicitud actualizada y aprobada correctamente.',
            ];
        } catch (Exception $e) {
            return [
                'status' => 500,
    ...
    public function Remesas_Clientes($ClienteId, $Dato)
    {
        ...
        FROM
            ...
            INNER JOIN cmx_orden_cargue oc ON ro.id_orden_cargue = oc.id
            INNER JOIN cmx_clientes cl ON oc.cli_id = cl.id
            INNER JOIN cmx_cotizaciones_serviciocliente coti ON cl.id=coti.id_cliente
            ...
            INNER JOIN cmx_ruta_puntosentrega pe1 ON oc.mer_idservicio = pe1.cod_ini_ruta AND oc.id_remitente = pe1.id
            INNER JOIN cmx_destinatarios_ss ds ON ss.nundoc_solicitud = ds.solicitud_servicio AND pe1.id_punto = ds.id_punto
    ...
    public function Detalle_Instrucciones_Remesas($InstruccionId)
    {
        ...
        FROM cmx_instruccion_facturacion ift 
            ...
            INNER JOIN cmx_ruta_puntosentrega pe1 ON oc.mer_idservicio = pe1.cod_ini_ruta AND oc.id_remitente = pe1.id
            INNER JOIN cmx_destinatarios_ss ds ON ss.nundoc_solicitud = ds.solicitud_servicio AND pe1.id_punto = ds.id_punto
            LEFT JOIN cmx_ajuste_valor_remesa ar on rm.id=ar.remesa_id AND ar.estado_ajuste='Aprobado'
            LEFT JOIN cmx_ajuste_valor_remesa ar on rm.id=ar.remesa_id
    ...
    public function Autorizar_Ajuste_Remesa($Remesa_Id, $valorFinal, $Valor_Actual, $selectOrigen, $selectDestino, $AjusteId, $DestinatarioId)
    {
        $nom_usuario = $_SESSION["usuario"]["nom_usuario"] ?? 'Sistema';
        $fecha = date('Y-m-d');
        $hora  = date('H:i:s');

        // Normalizar valorFinal para DECIMAL (evita formatos con comas)
        $valorFinal = is_numeric($valorFinal)
            ? number_format((float)$valorFinal, 2, '.', '')
            : null;
         ...
         * ====================================================== */
            ...
            SET estado_ajuste = 'Aprobado',
                autoriza = :autoriza,
                fecha_autorizacion = :fecha,
                hora_autorizacion = :hora
            WHERE id = :ajuste_id
              AND remesa_id = :remesa_id
              AND estado_ajuste = 'Pendiente'
        ");

            $sqlAjusteEstado->execute([
                ':autoriza'  => $nom_usuario,
                ':fecha'     => $fecha,
                ':hora'      => $hora,
                ':ajuste_id' => $AjusteId,
                ':remesa_id' => $Remesa_Id
            ...
            if ($sqlAjusteEstado->rowCount() === 0) {
                throw new Exception("No se encontró el ajuste pendiente con ID {$AjusteId} para la remesa {$Remesa_Id} o ya fue procesado.");
            }

            if ($valorFinal === null) {
                throw new Exception("El valor final no es numérico, no se puede aplicar el ajuste.");
            }

            /* ======================================================
         * 2) VALIDAR CONDICIÓN: ¿HAY ALGÚN DESTINATARIO CON TARIFA?
         *    - Si SÍ hay alguno (NO NULL) => actualizar SOLO dm.total_tarifa
         *    - Si NO hay ninguno (todos NULL) => actualizar SOLO ds.valor_tarifa (donde NULL)
         * ====================================================== */
            $sqlCheck = $this->_db3->prepare("
            SELECT COUNT(*) 
            FROM cmx_destinatarios_ss ds
            INNER JOIN cmx_solicitud_vehiculo2 ss  ON ds.solicitud_servicio = ss.nundoc_solicitud
            INNER JOIN cmx_remesa rm  ON ss.nundoc_solicitud = rm.mer_idservicio
            WHERE rm.id = :remesa_id
              AND ds.valor_tarifa IS NOT NULL
        ");
            $sqlCheck->execute([':remesa_id' => $Remesa_Id]);
            $hayTarifasEnDestinatarios = ((int)$sqlCheck->fetchColumn() > 0);

            if ($hayTarifasEnDestinatarios) {
                // echo "entro al if";

                /* ======================================================
             * CASO B: TODOS los destinatarios tienen valor_tarifa NULL
             * => ACTUALIZAR SOLO cmx_destinatarios_ss.valor_tarifa (solo NULL)
             * ====================================================== */
                $sqlUpdateDestinatarios = $this->_db3->prepare("
                    UPDATE cmx_remesa rm
                    SET rm.valor_tarifa = :tarifa
                    WHERE rm.id = :remesa_id
                ");
                $sqlUpdateDestinatarios->execute([
                    ':tarifa'    => $valorFinal,
                    ':remesa_id' => $Remesa_Id,
                ...
                /* ======================================================
             * CASO A: YA HAY valor_tarifa en destinatarios
             * => ACTUALIZAR SOLO cmx_detalle_mercancia2.total_tarifa
             * ====================================================== */
                $sqlUpdateDetalle = $this->_db3->prepare("
                UPDATE cmx_cotizaciones_serviciocliente coti
                INNER JOIN cmx_solicitud_vehiculo2 ss ON coti.n_cotizacion = ss.n_cotizacion
                INNER JOIN cmx_detalle_mercancia2 dm ON ss.idpareja_origen_destino = dm.id
                INNER JOIN cmx_remesa rm ON ss.nundoc_solicitud = rm.mer_idservicio
                SET dm.total_tarifa = :tarifa
                WHERE rm.id = :remesa_id
            ");
                $sqlUpdateDetalle->execute([
                    ':tarifa'    => $valorFinal,
                    ':remesa_id' => $Remesa_Id
                ]);

                // Nota: no forzamos rowCount > 0 si puede haber joins que no apliquen.
                // Si quieres, lo volvemos estricto.
            }
            ...
            return [
                'status'  => true,
                'message' => 'Ajuste autorizado y actualización aplicada según condición.'
            ];
        } catch (Exception $e) {
            $this->_db3->rollBack();
            ...
            return [
                'status'  => false,
                'message' => 'Error al autorizar ajuste: ' . $e->getMessage()
            ];
        }
    }
}
