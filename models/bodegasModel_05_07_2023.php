<?php
	class bodegasModel extends Model
	{
		...
		public function getTablaRemitentesDestinatarios($name){
			$sql = '
				SELECT 
					crd.*, 
		...
		public function getUbicacionesBodega($id_bodega){
			$sql = '
				SELECT 
					COUNT(cu.id) TOTAL,
					(SELECT COUNT(cu1.id) CUANTOS
					FROM cmx_ubicaciones cu1
					WHERE 
						cu1.id_bodega = cu.id_bodega
						AND cu1.estado != 3) UBICACIONES,
					(SELECT COUNT(cu1.id) CUANTOS
					FROM cmx_ubicaciones cu1
					WHERE 
						cu1.id_bodega = cu.id_bodega
						AND cu1.estado = 1) DISPONIBLES,
					(SELECT COUNT(cu1.id) CUANTOS
					FROM cmx_ubicaciones cu1
					WHERE 
						cu1.id_bodega = cu.id_bodega
						AND cu1.estado = 2) RESERVADAS,
					(SELECT COUNT(cu1.id) CUANTOS
					FROM cmx_ubicaciones cu1
					WHERE 
						cu1.id_bodega = cu.id_bodega
						AND cu1.estado = 0) OCUPADAS
				FROM 
					cmx_ubicaciones cu
				WHERE 
					cu.id_bodega = "' . $id_bodega . '";
			';
			$bodega = $this->_db->getConsulta($sql);
			return $bodega;
		}
		...
		public function getConfiguracionBodega($id_bodega){
			$sql = '
				SELECT
					...
					IF(
						(SELECT COUNT(ce1.id) FROM cmx_embalaje ce1 WHERE ce1.ubicacion = cu.id AND ce1.grupo > 0 AND ce1.sub_estado = 5),
						(
							SELECT CONCAT("<span>",cmc1.codigo," - ",cmc1.descripcion,"</span>") 
							FROM 
								cmx_embalaje ce1 
								INNER JOIN cmx_material_bodega cmb1 ON cmb1.id = ce1.id_material_bodega
			...
			';
			// echo "<p>" . $sql . "</p>";
			$configuracion = $this->_db->getConsulta($sql);
