...
class municipiosModel extends Model
{
    ...
    public function getHtmlSelect_pos()
    {
        ...
        if ($array) {
            $select = '<select class="form-control input-sm" id="slct_codigo_postal" aria-hidden="true">';
    ...
    public function crearDepto($pais, $depto)
    {
        ...
        if ($result) {
            $id_municipio = $result["rowsData"][0][0];
    ...
    public function getHtmlSelect($name, $id)
    {
        if ($name) {
            ...
            $sql = $this->_db3->prepare('SELECT * FROM cmx_municipios cm WHERE cm.depto IS NOT NULL AND cm.depto != "" AND cm.municipio IS NOT NULL AND cm.municipio != "" ORDER BY cm.pais, cm.depto, cm.municipio');
    ...
    public function getHtmlSelectnew()
    {
        ...
        $sql = 'SELECT * FROM cmx_municipios cm
					WHERE cm.depto IS NOT NULL AND cm.depto != ""
						AND cm.municipio IS NOT NULL AND cm.municipio != ""
					ORDER BY cm.pais, cm.depto, cm.municipio
				';
        // $array = $this->_db->getConsulta($sql);
        $sql = $this->_db3->prepare($sql);
        $sql->execute();
        $array = $sql->fetchAll(PDO::FETCH_ASSOC);

        $select = '<select class="form-control input-sm" name="slct_municipios" id="slct_municipios" aria-hidden="true">';
        $select .= '<option value="" disabled="disabled" selected= "selected">Seleccione</option>';
        // Se recorre contenido de la consulta
        if ($array) {
    ...
    public function getHtmlSelectnew2()
    {
        ...
        $select = '<select class="form-control input-sm" name="slct_municipios2" id="slct_municipios2" aria-hidden="true">';
        $select .= '<option value="" disabled="disabled" selected= "selected">Seleccione</option>';
        // Se recorre contenido de la consulta
        if ($array) {
            ...
            //     $select .= '<option value="' . $value[4] . '">    ' . $value[1] . ' (' . $value[2] . ' - ' . $value[3] . ')</option>';
            // }
            foreach ($array as $key => $value) {
                $select .= '<option value="' . $value['rndc_codigo_ciudad'] . '">    ' . $value['municipio'] . ' (' . $value['depto'] . ' - ' . $value['pais'] . ')</option>';
            }
    ...
    public function getHtmlSelect_sede($name, $id, $id_ciudad)
    {
        if ($name) {
            ...
            // Se recorre contenido de la consulta
            if ($array) {
                $select = '<select class="form-control input-sm" name="' . $name . '" id="slct_municipios_sede_' . $id . '" aria-hidden="true">';
                $select .= '<option value="" disabled="disabled" selected= "selected">Seleccione</option>';
                foreach ($array['rowsData'] as $key => $value) {
                    if ($value[0] == $id_ciudad || $_flag_rndc_id_municipio) {
                        $select .= '<option value="' . $value[0] . '" selected="">' . $value[1] . ' (' . $value[2] . ' - ' . $value[3] . ')</option>';
                        $rndc_id_municipio = $value[4];
                        $_flag_rndc_id_municipio = false;
                    } else {
                        $select .= '<option value="' . $value[0] . '">' . $value[1] . ' (' . $value[2] . ' - ' . $value[3] . ')</option>';
                    }
                }
                $select .= '</select>';
                $select .= '<input type="hidden" id="rndc_id_municipio" name="rndc_id_municipio" value=' . $rndc_id_municipio . ' >';
