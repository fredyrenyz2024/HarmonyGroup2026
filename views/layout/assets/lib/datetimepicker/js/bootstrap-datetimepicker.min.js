!(function(e) {
  ...
  var g = function(k, j) {
    ...
    this.format = d.parseFormat(
      j.format || this.element.data('date-format') || f[this.language].format || d.getDefaultFormat(this.formatType, 'input'),
      this.formatType,
    );
    this.isInline = false;
    ...
    this.bootcssVer =
      j.bootcssVer || (this.isInput ? (this.element.is('.form-control') ? 3 : 2) : (this.bootcssVer = this.element.is('.input-group') ? 3 : 2));
    ...
    if ('componentIcon' in j) {
      this.component = this.element.find('.input-group-addon ' + j.componentIcon).parent();
    }
    ...
    if ('navIcons' in j) {
      if (j.navIcons.rightIcon && j.navIcons.rightIcon) {
        this.icons = {leftArrow: j.navIcons.leftIcon, rightArrow: j.navIcons.rightIcon};
