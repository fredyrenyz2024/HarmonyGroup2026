...
if (document.getElementById("btn-login")) {
  ...
  btnLogin.addEventListener("click", (e) => {
    ...
    correo = formlogin.user_log.value.trim();
    password = formlogin.pass.value.trim();

    if (correo == "" && password == "") {
      mostrarError("Por favor ingrese su usuario/contraseña", msError);
      inputError([formlogin.user_log, formlogin.pass]);
      return false;
    } else {
      inputErrorRemove([formlogin.user_log, formlogin.pass]);
    }

    if (correo == "" || correo == null) {
      mostrarError("Por favor ingrese su correo", msError);
      inputError([formlogin.user_log]);
      formlogin.user_log.focus();
      return false;
    }
