...
window.initScript = function (id) {
  ...
  document.addEventListener("input", async (e) => {
    ...
    if (e.target.matches(`#campo-${window.VENTANA}-input_busqueda`) || e.target.matches(`#campo-${window.VENTANA}-input_busqueda *`)) {
      let filtro = document.getElementById(`campo-${window.VENTANA}-input_busqueda`).value.trim();
      Listar_trazabilidad('', '', '', filtro);
    }
  });
...
async function Listar_trazabilidad(fecha_inicial, fecha_final, valor, filtro) {
  ...
  dato.append('filtro', filtro === '' ? '' : filtro);
