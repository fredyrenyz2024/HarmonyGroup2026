...
(function () {
        ...
        function _renderKpis(d) {
            ...
            function calcularTiempos(d) {
                ...
                return {
                    ...
                    cargue_real: d.fecha_ejecutada_posicionamiento,
    ...
    window.initScript = function (id) {
        ...
        document.addEventListener("click", async (e) => {
            ...
            window.actualizarInfoRuta = function actualizarInfoRuta(legs) {
                console.log("🚀 ~ actualizarInfoRuta ~ legs:", legs)
                let totalDistanciaM = 0;
                let totalDuracionS = 0;
                legs.forEach((leg) => {
                    totalDistanciaM += leg.distance.value;
                    console.log("🚀 ~ actualizarInfoRuta ~ totalDistanciaM:", totalDistanciaM)
                    totalDuracionS += leg.duration.value;
                    console.log("🚀 ~ actualizarInfoRuta ~ totalDuracionS:", totalDuracionS)
                });
                ...
                // Actualizar badges distancia y duración
                ...
                // Cuando Maps responde, llamamos al nuevo endpoint con la
                // duración total bruta. El servicio aplica el 28 % de buffer,
                // ajusta horarios y devuelve ETA · SLA · estado de la operación.
                const pedidoId = window._tlSolicitudId;
                if (pedidoId) {
                    TiemposLogisticos.cargar(pedidoId, totalDuracionS);
                }
            }
            ...
            async function cargarGeocercasPedido(pedidoId) {
                ...
                try {
                    ...
                    // Calcular ruta inicial origen → destino.
                    // Si hay vehículo asignado, SatrackGPS recalculará automáticamente
                    // desde la posición real del vehículo en su primer tick,
                    // llamando a window.actualizarInfoRuta con los legs actualizados.
    ...
    async function actualizarLineaTiempo(
        ...
        const trazas = Array.isArray(window._trazabilidadPedido)
            ? window._trazabilidadPedido
            : [];
        const buscarTraza = (tipo) =>
            trazas.find((t) => t.tipo_trazabilidad === tipo) ?? null;
        ...
        if (dtLlegadaCargue) {
            setNodo(3, "done");
            if (dtCargue) {
                const dms = dtLlegadaCargue - dtCargue;
        ...
        const salidaEst = window._salidaCargueProgramada ?? null;
        if (dtSalidaCargue) {
            setNodo(4, "done");
            if (salidaEst) {
                const dms = dtSalidaCargue - salidaEst;
                setTime(
                    4,
                    `Sal. est.: ${fmt(salidaEst)} | Real: ${fmt(dtSalidaCargue)}`,
                );
                setDelta(4, Math.abs(dms) > 60000 ? dms : null);
            } else if (dtCargue) {
                const dms = dtSalidaCargue - dtCargue;
            ...
            if (salidaEst) {
                setTime(
                    4,
                    `Llegada: ${fmt(dtCargueReal)} | Salida est.: ${fmt(salidaEst)}`,
                );
                setDelta(4, null);
            } else if (dtCargue) {
                const dms = dtCargueReal - dtCargue;
            ...
            setTime(
                4,
                salidaEst
                    ? `Llegada: ${fmt(dtLlegadaCargue)} | Salida est.: ${fmt(salidaEst)}`
                    : dtCargue
                        ? `Prog: ${fmt(dtCargue)}`
                        : "—",
            );
            ...
            setTime(
                4,
                salidaEst
                    ? `Prog llegada: ${fmt(dtCargue)} | Salida est.: ${fmt(salidaEst)}`
                    : `Prog: ${fmt(dtCargue)}`,
            );
        } else {
            setNodo(4, "pending");
        ...
        const pesos = [
            ...
            },
            {
                pct: dtLlegadaDes
                    ? 100
                    : dtTransito || dtInicioRuta || dtSalidaCargue
                        ? 50
                        : 0,
            },
    ...
    function renderTablaResumen(container, tareas, titulo) {
        ...
        const totalAsig = tareas.reduce(
            (a, t) => a + (t._tiempos?.minAsignados || 0),
            0,
        );
        const totalReal = tareas.reduce(
            (a, t) =>
                t.estado === "COMPLETADO" && t.tiempo_real_gestion != null
                    ? a + t.tiempo_real_gestion
                    : a,
            0,
        );
        const totalDelta = tareas.reduce(
            (a, t) =>
                t.estado === "COMPLETADO" && t.tiempo_real_min != null
                    ? a + t.tiempo_real_min
                    : a,
            0,
        );
        ...
        const totalColorD =
            totalDelta === 0 ? "#0369a1" : totalDelta > 0 ? "#16a34a" : "#dc2626";
        const totalDeltaHtml =
            totalCompletadas > 0
                ? `<span style="font-size:9px;font-weight:700;font-family:'JetBrains Mono',monospace;padding:1px 6px;border-radius:7px;background:${totalColorD}12;color:${totalColorD};border:1px solid ${totalColorD}30">${totalDelta > 0 ? "+" : totalDelta < 0 ? "-" : "="}${fmtMin(Math.abs(totalDelta))}</span>`
                : "—";
            ...
            <table class="oc-table" style="margin-bottom:.8rem">
                ...
                <tfoot>
                    <tr class="total">
                        ...
                        <td class="v" style="text-align:center">${totalReal > 0 ? fmtMin(totalReal + totalAsig) : "—"}</td>
