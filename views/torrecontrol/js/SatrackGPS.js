...
//         // console.log("🚀 ~ _actualizarMarcador ~ vehiculo:", vehiculo)
...
//         _recalcularRuta(posicion, 1);
//         // console.log("HOLA MUNDO DESDE EL ULTIMO");
...
//      ══════════════════════════════════════════════════════════*/
//     function _recalcularRuta(posVehiculo, _intentos) {
//         // console.log("🚀 ~ _recalcularRuta ~ _intentos:", _intentos)
//         console.log("🚀 ~ _recalcularRuta ~ window.MAPA_CONFIG:", window.MAPA_CONFIG)
//         if (!_directionsRenderer || !window.MAPA_CONFIG) return;

//         const { origen, destino } = window.MAPA_CONFIG;
//         console.log("🚀 ~ _recalcularRuta ~ destino:", destino)
//         console.log("🚀 ~ _recalcularRuta ~ origen:", origen)

//         // Si el destino aun no esta listo (cargarGeocercasPedido sigue cargando),
//         // reintentar hasta 10 veces cada 300 ms antes de rendirse.
//         if (!destino?.lat) {
//             const intentos = _intentos ?? 0;
//             if (intentos < 10) setTimeout(() => _recalcularRuta(posVehiculo, intentos + 1), 300);
//             return;
//         }
//         console.log("🚀 ~ _recalcularRuta ~ posVehiculo:", posVehiculo)

//         // Validamos si posVehiculo tiene coordenadas reales (!= 0 y != null)
//         let latOrigen, lngOrigen;
//         const _latV = typeof posVehiculo?.lat === 'number' ? posVehiculo.lat : null;
//         const _lngV = typeof posVehiculo?.lng === 'number' ? posVehiculo.lng : null;
//         console.log('[SatrackGPS] _recalcularRuta posVehiculo:', posVehiculo, '_latV:', _latV, '_lngV:', _lngV, 'destino:', destino);
//         if (_latV !== null && _lngV !== null && _latV !== 0 && _lngV !== 0) {
//             latOrigen = _latV;
//             lngOrigen = _lngV;
...
//             }, (result, status) => {
//                 console.log("🚀 ~ _recalcularRuta ~ status:", status)
//                 if (status === 'OK') {
//                     _directionsRenderer.setDirections(result);
//                     // Fuente 'gps': el origen del cálculo es la posición real del vehículo
//                     if (typeof actualizarInfoRuta === 'function') {
//                         actualizarInfoRuta(result.routes[0].legs, 'gps');
//                     }
//                 } else {
...
//             console.log('[SatrackGPS] _tick vehiculo:', JSON.stringify(vehiculo));
...
window.SatrackGPS = (() => {
    ...
    /* ══════════════════════════════════════════════════════════
     ...
     ══════════════════════════════════════════════════════════*/
    function _recalcularRuta(posVehiculo, _intentos) {
        if (!window.MAPA_CONFIG) return;

        const { origen, destino } = window.MAPA_CONFIG;

        // Si el destino aun no esta listo, reintentar hasta 10 veces cada 300 ms.
        if (!destino?.lat) {
            const intentos = _intentos ?? 0;
            if (intentos < 10) setTimeout(() => _recalcularRuta(posVehiculo, intentos + 1), 300);
            return;
        ...
        // Coordenadas reales del vehiculo (!= 0 y != null).
        // Si no son validas se usa el origen del remitente como fallback.
        const _latV = typeof posVehiculo?.lat === 'number' ? posVehiculo.lat : null;
        const _lngV = typeof posVehiculo?.lng === 'number' ? posVehiculo.lng : null;
        const latCalculo = (_latV && _lngV && _latV !== 0 && _lngV !== 0) ? _latV : origen?.lat;
        const lngCalculo = (_latV && _lngV && _latV !== 0 && _lngV !== 0) ? _lngV : origen?.lng;
        const fuente = (_latV && _lngV && _latV !== 0 && _lngV !== 0) ? 'gps' : 'remitente';

        if (!latCalculo || !lngCalculo) return;

        // Calcular distancia y tiempo vehiculo -> destino SIN pintar la ruta.
        // La ruta visual origen -> destino ya esta pintada por cargarGeocercasPedido
        // y no debe sobreescribirse.
        google.maps.importLibrary('routes').then(({ DirectionsService, TravelMode }) => {
            new DirectionsService().route({
                origin: { lat: latCalculo, lng: lngCalculo },
                destination: { lat: destino.lat, lng: destino.lng },
                travelMode: TravelMode.DRIVING,
            }, (result, status) => {
                if (status === 'OK') {
                    // Solo actualizar badges y tarjetas SLA; NO llamar setDirections.
                    if (typeof actualizarInfoRuta === 'function') {
                        actualizarInfoRuta(result.routes[0].legs, fuente);
                    }
                } else {
                    console.warn('SatrackGPS - Directions error:', status);
                }
