...
window.initScript = function (id) {
  ...
  document.addEventListener("click", async (e) => {
    ...
    if (e.target.matches('#rc-btn-buscar') || e.target.matches('#rc-btn-buscar *')) {
      const fecha_inicial = document.getElementById('rc-fecha-inicial')?.value ?? '';
      const fecha_final = document.getElementById('rc-fecha-final')?.value ?? '';
      const estadoRecurso = document.getElementById('rc-estado-recurso')?.value ?? '';
      const modalidad = document.getElementById('rc-modalidad')?.value ?? '';
      const proceso = document.getElementById('rc-proceso')?.value ?? '';

      document.querySelectorAll('.rc-kpi-resumen').forEach(c => c.classList.remove('active'));

      listar_recursos_proveedor(fecha_inicial, fecha_final, window.VENTANA, estadoRecurso, modalidad, proceso);
    }
  ...
  // ═══════════════════════════════════════════════════
  // KPI CARDS — clic para filtrar por estado_recurso
  // ═══════════════════════════════════════════════════
  document.querySelectorAll('.rc-kpi-resumen').forEach(function (card) {
    card.addEventListener('click', function () {
      ...
      if (estado === 'todos') {
        listar_recursos_proveedor(fi, ff, window.VENTANA, '', '', '');
      } else {
        listar_recursos_proveedor(fi, ff, window.VENTANA, estado, '', '');
      }
    });
  });
  ...
  document.getElementById('rc-btn-limpiar')?.addEventListener('click', function () {
    ['rc-estado-recurso', 'rc-modalidad', 'rc-proceso'].forEach(function (id) {
      const el = document.getElementById(id); if (el) el.value = '';
    });
    const fi = document.getElementById('rc-fecha-inicial');
    const ff = document.getElementById('rc-fecha-final');
    if (fi) fi.value = '';
    if (ff) ff.value = '';
    document.querySelectorAll('.rc-kpi-resumen').forEach(function (c) { c.classList.remove('active'); });
...
async function listar_recursos_proveedor(fecha_inicial, fecha_final, ventana, estadoRecurso, modalidad, proceso) {
  ...
  dato.append('modalidad', modalidad ?? '');
  dato.append('proceso', proceso ?? '');
  try {
    ...
    if (data) {
      ...
      if (rcPagBar) {
        ...
        const s = function (id, v) { const n = document.getElementById(id); if (n) n.textContent = v; };
...
// ─── Actualiza los KPI cards con conteos de la data de recursos ──────────
function rc_actualizarKpis(data) {
  ...
  data.forEach(function (el) {
    if (el.estado_recurso === 'Pendiente Iniciar') c.pendiente++;
    if (el.estado_recurso === 'Iniciado') c.iniciado++;
    if (el.estado_recurso === 'Completado') c.completado++;
    if (el.estado_recurso === 'Cancelado' ||
      el.estado_recurso === 'Rechazado') c.cancelado++;
  });
