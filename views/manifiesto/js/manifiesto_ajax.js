...
async function Validar_Tarifa() {
  ...
  try {
    const response = await fetch($('#id_url_ajax').val() + 'web_service/Validar_Sicetac', {
      method: 'POST',
      body: datos,
      cache: 'no-cache',
    });
  ...
  if (data.status === 'true') {
    ...
    if (data.resultado && Array.isArray(data.resultado.documento)) {
      ...
      data.resultado.documento.forEach((documento, index) => {
        ...
        // console.log(`Nombre de unidad transporte: ${documento.nombreunidadtransporte}`);
        // console.log(`Nombre tipo de carga: ${documento.nombretipocarga}`);
        // console.log(`Nombre ruta: ${documento.nombreruta}`);
        console.log(`Valor: ${documento.valor}`);
        // console.log(`Valor tonelada: ${documento.valortonelada}`);
        // console.log(`Valor hora: ${documento.valorhora}`);
        // console.log(`Distancia: ${documento.distancia}`);
        console.log('--------------------------------');
      });
    } else {
...
//Fin de la validacion de los valrfes del sicetac
