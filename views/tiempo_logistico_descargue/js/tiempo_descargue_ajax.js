$(document).ready(function() {
  ...
  let rm = params1.get('remesa');
  Buscar_Dato(mnf);
  $('#remesas').val(rm);

  /* Fecha y Hora de Llegada de descargue */
  ...
  hirllegada.addEventListener('input', function() {
    ...
    if (value.length === 5) {
      ...
      if (hours > 23 || minutes > 59) {
        ...
        let fehca_llegada_descargue = $('#firentrada');
        let hora_llegada_descargue = $('#hirentrada');
        fehca_llegada_descargue.prop('disabled', true);
        hora_llegada_descargue.prop('disabled', true);
        // hora_llegada_descargue.focus();
      } else {
        error.textContent = '';
        let fehca_llegada_descargue = $('#firentrada');
        let hora_llegada_descargue = $('#hirentrada');
        fehca_llegada_descargue.prop('disabled', false);
        hora_llegada_descargue.prop('disabled', false);
      }
    } else {
      error.textContent = '';
      let fehca_llegada_descargue = $('#firentrada');
    ...
    function(data) {
      if (data) {
        data.forEach(element => {
          console.log(element.tipo_fecha);
          if (element.tipo_fecha === 'fec_llegada') {
            $('#firllegada').val(element.fecha_descargue);
            $('#hirllegada').val(element.hora_descargue);
            $('#firllegada').attr('disabled', true);
            $('#hirllegada').attr('disabled', true);
          }
          if (element.tipo_fecha === 'fec_entrada') {
            $('#firentrada').val(element.fecha_descargue);
            $('#hirentrada').val(element.hora_descargue);
            $('#firentrada').attr('disabled', true);
            $('#hirentrada').attr('disabled', true);
          }
          if (element.tipo_fecha === 'fec_salida') {
            $('#firsalida').val(element.fecha_descargue);
            $('#hirsalida').val(element.hora_descargue);
  ...
  $.post(
    ...
    'manifi=' + mnf + '&remesa=' + rm,
    function(data) {
      if (data) {
        localStorage.setItem('ordenes_descargue', JSON.stringify(data));
        var dato = JSON.parse(localStorage.getItem('ordenes_descargue'));
        ordremesas = dato;
        $.each(dato, function(index, dato) {
          $('#placa').val(dato.placa);
        });
        // console.log($('#placa').val());
        var datos = JSON.parse(localStorage.getItem('ordenes_descargue'));
        for (var i = 0; i < datos.length; i++) {
          // if (id === datos[i].id) {}
          $('.remesa_id').html(datos[i].id);
          $('.peso').html(datos[i].peso);
          $('.empaque').html(datos[i].empaque);
          $('.cliente').html(datos[i].Cliente);
          $('.remitente').html(datos[i].Remitente);
          $('.destinatario').html(datos[i].Destinatario);
          $('.placa').html(datos[i].placa);
          $('.fecha_entrega').html('0000-00-00 00-00-00');
          $('.volumen').html(datos[i].mer_volumen);
          $('.mercancia').html(datos[i].mer_producto);
          $('.direccion_remitente').html(datos[i].Direccion_Remitente);
          $('.direccion_destinatario').html(datos[i].Direccion_Destinatario);
        }
      }
    },
    'json',
  ...
  $('#hirllegada').change(function(event) {
    ...
    if (!fecha || !hora) {
      console.log('Fecha o hora no definidas');
      return;
    }
    ...
    if (fechaYHora.isAfter(fechaActualServidor)) {
      ...
      var mensaje = 'La hora de llegada no puede ser mayor que la actual en el servidor.';
      Swal.fire({
        // position: 'top-end',
        position: 'center',
        icon: 'warning',
      ...
      console.log('La hora ingresada es válida.');
    }
  });
...
function Registrar_Tiempo() {
  if (window.confirm('¿Esta seguro de registrar los tiempos de descargue?')) {
    $('#manifiestos').val();
    ...
    paquete =
      ...
      '&num_orden=' +
      orden;
    $.post($('#id_url_ajax').val() + 'tiempo_logistico_descargue/Registro_Tiempo_Descargue', paquete, function(data) {
