...
async function listar_pedidos_proveedor(fecha_inicial, fecha_final) {
  ...
  try {
    ...
    if (data) {
      ...
      let btn_publicacion = "";
      let col_prioridad = "";
      let btn_iniciar_pedido = "";

      data.forEach(element => {
        const fila = document.createElement('tr');

        if (element.estado_proceso_pedido === 'Pendiente Iniciar') {
          col_estatus_publicacion = ` <span class="badge badge-phoenix fs-10 badge-phoenix-secondary"><span class="badge-label">${element.estado_proceso_pedido}</span><span class="ms-1" data-feather="plus" style="height:12.8px;width:12.8px;"></span></span>`;
          btn_iniciar_pedido = `<a class="dropdown-item fw-bold" id="btn_iniciar_pedido" href="#" data-id="${element.numdoc_solicitud}"> <span class="uil uil-play"></span> Inicar Pedido </a>`;
        } else if (element.estado_proceso_pedido === 'Iniciado') {
          col_estatus_publicacion = ` <span class="badge badge-phoenix fs-10 badge-phoenix-info"><span class="badge-label">${element.estado_proceso_pedido}</span><span class="ms-1" data-feather="plus" style="height:12.8px;width:12.8px;"></span></span>`;
        } else if (element.estado_proceso_pedido === 'Cancelado') {
          col_estatus_publicacion = ` <span class="badge badge-phoenix fs-10 badge-phoenix-danger"><span class="badge-label">${element.estado_proceso_pedido}</span><span class="ms-1" data-feather="plus" style="height:12.8px;width:12.8px;"></span></span>`;
        ...
        } else if (element.estado_proceso_pedido === 'Completado') {
          col_estatus_publicacion = `<span class="badge badge-phoenix fs-10 badge-phoenix-success"><span class="badge-label">${element.estado_proceso_pedido}</span><span class="ms-1" data-feather="plus" style="height:12.8px;width:12.8px;"></span></span>`;
        } else if (element.estado_proceso_pedido === 'Rechazado') {
          col_estatus_publicacion = `<span class="badge badge-phoenix fs-10 badge-phoenix-danger"><span class="badge-label">${element.estado_proceso_pedido}</span><span class="ms-1" data-feather="plus" style="height:12.8px;width:12.8px;"></span></span>`;
        }
        ...
        <div class="dropdown">
          ...
          ${btn_iniciar_pedido}
           ...
           <!--
            ...
            <a class="dropdown-item fw-bold" id="btn-solicitar-prioridad" href="#" data-id="${element.numdoc_solicitud}" data-id2="${element.numdoc_solicitud}"> <span class="uil uil-bell"></span> Solicitar Prioridad </a>-->
          </div>
        </div>
        `;
