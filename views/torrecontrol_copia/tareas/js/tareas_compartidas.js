...
async function Listar_pedidos() {
    await fetch($("#base_url").val() + "torrecontrol/Comportaidos_Conmigo", {
        ...
        .then((response) => {
            ...
            console.log("🚀 ~ Detalles_actividaes ~ response:", response[0].total_registros);

            document.querySelector(".contador").innerHTML = response[0].total_registros;
            // Actualizar badge de contador
            // $('.contador').html(response.total_registros ?? 0);
            ...
            response.forEach((element) => {
                ...
                // columnaReferencia.innerHTML = `<a href="#!" class="text-decoration-none" data-bs-toggle="offcanvas" data-bs-target="#offcanvasTop" aria-controls="offcanvasTop" id="btn_detalle_pedido" data-id="${element.recurso_id}">${element.referencia}</a>`;
                // columnaReferencia.innerHTML = `<a href="${$("#base_url").val()}pedidos/detalle_compartidos/${element.numdoc}/${valores}">${element.referencia}</a>`;
