/**
 * generacion_automaticas_ventas.js
 * ═══════════════════════════════════════════════════════════════════════════
 * REQ 19v — Generación Automática de Rutas para Actualizar (VENTAS)
 * REQ 20v — Parámetro TOP % de Viajes (VENTAS)
 *
 * CAMBIOS vs versión anterior:
 *   - Layout corregido: columna izquierda sidebar + columna derecha stats+tabla
 *   - Modal multi origen/destino: SIN botón agregar, SOLO mostrar detalles
 *     existentes con municipio como texto de solo lectura y rate_1/rate_2 editables
 *   - Vigencia: fecha_inicio / fecha_fin (en lugar de año/mes como en costos)
 * ═══════════════════════════════════════════════════════════════════════════
 */
...
(function () {
  ...
  function _initFechas() {
    ...
    // Fechas vigencia modal
    _v('m-fecha-inicio', hoy.toISOString().slice(0, 10));
  }
  ...
  window.ventasRP.guardarParametro = function () {
    ...
    if (isNaN(val) || val < 1 || val > 100) { _toast('El parámetro debe estar entre 1 y 100.', 'warning'); return; }
    fetch(`${EP()}/parametro`, {
  ...
  window.ventasRP.setTab = function (tab) {
    S.currentEstado = tab;
    document.querySelectorAll('.vta-tab-btn').forEach(b => b.classList.remove('active'));
  ...
  function _renderTabla() {
    ...
    const wrap = document.getElementById('vta-table-wrap');
    if (!wrap) return;

    _s('count-txt', `${data.length} rutas`);

    if (!data.length) {
      wrap.innerHTML = `<div class="vta-empty">
        <div class="vta-empty-icon"><i class="fas fa-tags"></i></div>
        <div class="vta-empty-text">No hay rutas para mostrar con los filtros aplicados.</div>
      </div>`;
      return;
    }

    const rows = data.map((d, idx) => {
      const chk = `<input type="checkbox" onchange="ventasRP.toggleCheck(${idx})" ${S.selected.has(idx) ? 'checked' : ''}>`;
      ...
      const diasStr = d.dias_sin_actualizar != null
        ? `<span style="font-size:.66rem;color:var(--muted)">${d.dias_sin_actualizar}d</span>` : '—';
      const clienteLbl = d.cliente_tarifa_id
        ? `<span style="font-size:.63rem;background:#d1fae5;color:#065f46;padding:1px 5px;border-radius:3px">${S.clienteMap[d.cliente_tarifa_id] || 'ID:' + d.cliente_tarifa_id}</span>`
        : `<span style="font-size:.63rem;color:var(--muted)">General</span>`;

      // Vigencia: fecha_fin en lugar de año/mes
      const vigencia = d.fecha_fin
        ? `<span style="font-size:.63rem;color:var(--muted)">${d.fecha_fin}</span>`
        : '—';
      ...
      return `<tr>
        ...
        <td>
          ...
          <div style="font-size:.6rem;color:var(--muted)">${_esc(d.depto_origen)} · ${_esc(d.zona_origen || '—')}</div>
        </td>
        <td>
          <div style="font-weight:700;color:var(--navy);font-size:.78rem">${_esc(d.nombre_destino)}</div>
          <div style="font-size:.6rem;color:var(--muted)">${_esc(d.depto_destino)} · ${_esc(d.zona_destino || '—')}</div>
        </td>
        <td><span style="font-family:'DM Mono',monospace;font-size:.73rem;font-weight:700">${_esc(d.tipo_vehiculo)}</span></td>
        <td>${tripBar}</td>
        <td>${plena}</td>
        <td>${estTag}</td>
        <td>${diasStr}</td>
        ...
        <!--<td style="white-space:nowrap;">
          <button class="btn btn-primary btn-sm" onclick="ventasRP.abrirModal(${idx})" title="Editar tarifa">
    ...
    wrap.innerHTML = `<table class="vta-table" id="tabla-ventas-prioritarias">
      <thead>
        <tr>
      ...
      <tbody>${rows}</tbody>
      <tfoot><tr><td colspan="11" style="text-align:right;font-style:italic">${data.length} rutas · Tarifas de Venta</td></tr></tfoot>
    </table>`;
    ...
    $t.DataTable({
      ...
      initComplete: function () {
        ...
        api.columns().every(function (ci) {
          ...
          $(cell).html(`<input type="text" placeholder="${title}" style="font-size:10px;height:22px;width:100%;box-sizing:border-box;border:1px solid #ddd;border-radius:3px;padding:0 4px;">`);
  ...
  window.ventasRP.toggleTodos = function (cb) {
    ...
    _updateSelCount(); _renderTabla();
  };
  window.ventasRP.toggleSelTodo = function () {
    S.selTodoActivo = !S.selTodoActivo;
    const data = S.filteredData.length ? S.filteredData : S.allData;
    data.forEach((_, i) => { if (S.selTodoActivo) S.selected.add(i); else S.selected.delete(i); });
    _updateSelCount(); _renderTabla();
  };
  window.ventasRP.deseleccionarTodo = function () { S.selected.clear(); _updateSelCount(); _renderTabla(); };
  ...
  /* ══════════════════════════════════════════════════════════
   | MODAL — ABRIR
   | Solo muestra detalles existentes — sin botón Agregar
   ══════════════════════════════════════════════════════════ */
  window.ventasRP.abrirModal = function (idx) {
    ...
    // Ocultar secciones multi hasta que carguen los detalles
    _showSection('section-multi-origen', false);
    _showSection('section-multi-destino', false);
    document.getElementById('multi-origen-rows').innerHTML = '';
    document.getElementById('multi-destino-rows').innerHTML = '';

    const modal = document.getElementById('modal-editar');
    if (modal) modal.style.display = 'flex';

    // Cargar detalles reales del backend si hay tarifa_id
    if (d.tarifa_id) _cargarDetallesModal(d.tarifa_id);
  };

  /* ── [F] CARGAR DETALLES — solo leer, sin agregar ── */
  function _cargarDetallesModal(tarifaId) {
    fetch(`${EP()}/detalle/${tarifaId}?empresa_id=${EMPRESA()}`, { headers: HEADERS() })
      ...
      .then(res => {
        ...
        // Vigencia desde el detalle del backend
        if (det.fecha_inicio) _v('m-fecha-inicio', det.fecha_inicio);
        if (det.fecha_fin) _v('m-fecha-fin', det.fecha_fin);

        S.multiOrigenRows = det.multi_origen_detalles || [];
        S.multiDestinoRows = det.multi_destino_detalles || [];

        // Multi Origen: mostrar solo si tiene detalles
        _showSection('section-multi-origen', S.multiOrigenRows.length > 0);
        _renderDetallesReadonly('origen', S.multiOrigenRows);

        // Multi Destino: mostrar solo si tiene detalles
        _showSection('section-multi-destino', S.multiDestinoRows.length > 0);
        _renderDetallesReadonly('destino', S.multiDestinoRows);
      })
      .catch(() => { });
  }

  /**
   * Renderiza las filas de multi origen/destino:
   *   - Municipio: texto de solo lectura (nombre visible)
   *   - Rate 1 y Rate 2: inputs numéricos editables
   *   - SIN botón eliminar ni agregar
   */
  function _renderDetallesReadonly(tipo, arr) {
    const cont = document.getElementById(`multi-${tipo}-rows`);
    if (!cont) return;

    if (!arr.length) { cont.innerHTML = ''; return; }

    cont.innerHTML = arr.map((row, i) => `
      <div style="display:grid;grid-template-columns:1fr 90px 90px;gap:6px;align-items:center;margin-bottom:5px;padding:7px 8px;background:#fff;border-radius:6px;border:1px solid var(--border);">
        <!-- Municipio: solo lectura -->
        <div style="font-size:.75rem;font-weight:600;color:var(--navy);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="${_esc(row.nombre_municipio || row.municipio_id || '')}">
          <i class="fas fa-map-pin" style="color:#059669;font-size:.6rem;margin-right:3px"></i>
          ${_esc(row.nombre_municipio || row.municipio_id || '—')}
          <div style="font-size:.6rem;color:var(--muted);font-weight:400">${_esc(row.depto || '')}</div>
        </div>
        <!-- Rate 1: editable -->
        <div style="position:relative;">
          <span style="position:absolute;left:6px;top:50%;transform:translateY(-50%);font-size:.63rem;color:var(--muted);font-weight:700">$</span>
          <input type="number" min="0" step="1000"
            value="${row.rate_1 || 0}"
            style="padding-left:16px;font-size:.73rem;font-family:'DM Mono',monospace;width:100%;border:1px solid var(--border);border-radius:5px;padding-top:5px;padding-bottom:5px;outline:none;"
            oninput="ventasRP._updateDetalle('${tipo}',${i},'rate_1',parseFloat(this.value)||0)"
            onfocus="this.style.borderColor='#059669'" onblur="this.style.borderColor='var(--border)'">
        </div>
        <!-- Rate 2: editable -->
        <div style="position:relative;">
          <span style="position:absolute;left:6px;top:50%;transform:translateY(-50%);font-size:.63rem;color:var(--muted);font-weight:700">$</span>
          <input type="number" min="0" step="1000"
            value="${row.rate_2 || 0}"
            style="padding-left:16px;font-size:.73rem;font-family:'DM Mono',monospace;width:100%;border:1px solid var(--border);border-radius:5px;padding-top:5px;padding-bottom:5px;outline:none;"
            oninput="ventasRP._updateDetalle('${tipo}',${i},'rate_2',parseFloat(this.value)||0)"
            onfocus="this.style.borderColor='#059669'" onblur="this.style.borderColor='var(--border)'">
        </div>
      </div>`).join('');
  }

  window.ventasRP._updateDetalle = function (tipo, idx, field, val) {
  ...
  window.ventasRP.guardarEdicion = function () {
    ...
    // Recoger detalles editados (solo los que ya existen — con detalle_id)
    const multiOrigenDetalles = S.multiOrigenRows.map(r => ({
      detalle_id: r.detalle_id || null,
      municipio_id: r.municipio_id || null,
      rate_1: parseFloat(r.rate_1) || 0,
      rate_2: parseFloat(r.rate_2) || 0,
    }));
    const multiDestinoDetalles = S.multiDestinoRows.map(r => ({
      detalle_id: r.detalle_id || null,
      municipio_id: r.municipio_id || null,
      rate_1: parseFloat(r.rate_1) || 0,
      rate_2: parseFloat(r.rate_2) || 0,
    ...
    const multiOrigenTotal = multiOrigenDetalles.reduce((s, r) => s + r.rate_1, 0);
    const multiDestinoTotal = multiDestinoDetalles.reduce((s, r) => s + r.rate_1, 0);
    ...
    const payload = {
      ...
      fecha_inicio: document.getElementById('m-fecha-inicio')?.value || null,
      fecha_fin: document.getElementById('m-fecha-fin')?.value || null,
    };

    fetch(`${EP()}/actualizar`, { method: 'POST', headers: HEADERS(), body: JSON.stringify(payload) })
      .then(r => r.json())
      .then(res => {
        ...
        if (res.success) {
          ...
          d.fecha_fin = document.getElementById('m-fecha-fin')?.value || d.fecha_fin;
          _renderTabla();
          _recalcStats();
  ...
  window.ventasRP.actualizarSeleccionadas = function () {
    ...
    if (!confirm(`¿Actualizar ${sels.length} tarifas de venta seleccionadas?`)) return;

    const tarifas = sels.map(d => ({
      origen: d.origen, destino: d.destino, tipo_vehiculo: d.tipo_vehiculo,
      plena: d.tarifa_plena || 0, multi_recogida: d.multi_recogida || 0,
      multi_origen: d.multi_origen || 0, multi_entrega: d.multi_entrega || 0,
      multi_destino: d.multi_destino || 0, cliente_id: d.cliente_tarifa_id || null,
    }));
    ...
    fetch(`${EP()}/actualizar-masivo`, {
      ...
      .then(r => r.json())
      .then(res => {
        _toast(res.message || (res.success ? 'Proceso completado.' : 'Error.'), res.success ? 'success' : 'error');
  ...
  window.ventasRP.exportarExcel = function () {
    ...
    fetch(`${EP()}/exportar`, { method: 'POST', headers: HEADERS(), body: JSON.stringify({ data, ruta: 'para_actualizar' }) })
      .then(r => r.json())
      .then(res => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Rutas_Ventas');
        XLSX.writeFile(wb, `Rutas_Ventas_Prioritarias_${new Date().toISOString().slice(0, 10)}.xlsx`);
  ...
  function _clearStats() {
    ['st-rutas', 'st-sin', 'st-venc', 'st-porv', 'st-vig', 'st-viajes'].forEach(id => _s(id, '—'));
  }
  function _renderParetoBar(r) {
    ...
    _t('pareto-fill-label', `TOP ${r.parametro_top} · ${r.semaforo_rojo + r.semaforo_amarillo} requieren acción`);
    ...
    _t('pareto-rutas-total', `${total} rutas priorizadas`);
  }
  ...
  function _buildPayload() {
    ...
    if (desde > hasta) { _toast('Fecha Desde no puede ser mayor a Hasta.', 'warning'); return null; }
  ...
  function _showLoading(msg) {
    const w = document.getElementById('vta-table-wrap');
    if (w) w.innerHTML = `<div class="vta-loading"><div class="vta-spinner"></div><div style="font-size:.75rem;color:var(--muted);margin-top:8px">${msg}</div></div>`;
  }
  function _showError(msg) {
    const w = document.getElementById('vta-table-wrap');
    if (w) w.innerHTML = `<div class="vta-empty"><div class="vta-empty-icon" style="color:var(--red)"><i class="fas fa-exclamation-triangle"></i></div><div class="vta-empty-text" style="color:var(--red)">${_esc(msg)}</div></div>`;
  }
  ...
  function _semBadge(s) {
    const map = {
      rojo: '<span class="badge badge-rojo"><i class="fas fa-circle" style="font-size:.45rem"></i> Crítico</span>',
      amarillo: '<span class="badge badge-amarillo"><i class="fas fa-circle" style="font-size:.45rem"></i> Alerta</span>',
      verde: '<span class="badge badge-verde"><i class="fas fa-circle" style="font-size:.45rem"></i> OK</span>',
    };
  ...
  function _estadoTag(e) {
    ...
    return map[e] || `<span style="font-size:.63rem">${_esc(e)}</span>`;
  }
  function _tripBar(viajes, max) {
    const pct = max > 0 ? Math.min(100, Math.round((viajes / max) * 100)) : 0;
    return `<div class="trip-bar"><div class="trip-bar-track"><div class="trip-bar-fill" style="width:${pct}%"></div></div><span class="trip-bar-num">${viajes}</span></div>`;
  }
