<?php 
	...
	switch ( $_GET["action"] ) {
		...
		case 'agregar_cotizacion':
			...
			if ( $file["error"] == 0) {
				...
				if ( move_uploaded_file($tmp_file , $archivo_temporal) ) {
					...
					if (!file_exists($carpeta_destino_1)) { mkdir($carpeta_destino_1, 0777, true); }

					$carpeta_destino_2 = $carpeta_destino_1 . "/" . $_POST["id_intr_proyecto"];
					if (!file_exists($carpeta_destino_2)) { mkdir($carpeta_destino_2, 0777, true); }

					$carpeta_destino_3 = $carpeta_destino_2 . "/" . $_POST["div"];
					if (!file_exists($carpeta_destino_3)) { mkdir($carpeta_destino_3, 0777, true); }

					$destino = $carpeta_destino_3 . "/" . $archivo;

					if(copy($archivo_temporal,$destino)){ $return["copy_file_result"] = true;
					}else { $return["copy_file_result"] = false; }
				}
				if (file_exists($archivo_temporal)) { unlink($archivo_temporal); }
			}else{ $return["copy_file_result"] = false; }
			break;
		...
		case 'editar_cotizacion':
			...
			if ( isset($cotizacion["general"]) AND $cotizacion["general"] ) {
				...
				$info_basica = $cotizacion["general"]["rowsData"][0];
				...
				foreach ($cotizacion["tramos"]["tramos"]["rowsData"] as $key => $value) {
					$_origen.= '
						<tr>
				...
				$_oferta_content = '';
				if ( $_POST["id_perfil"] == 13 OR $_POST["id_perfil"] == 1 OR $_POST["id_perfil"] == 25 ) {
					$_oferta_content = '
					...
					if ( $cotizacion["oferta_comercial"] ) {
						$oferta = $cotizacion["oferta_comercial"]["rowsData"][0];
						$_oferta_content = '
				...
				$accordion_content = '';
				foreach ($conceptos["rowsData"] as $key => $value) {
					...
					if ($value[0] != 11) {
						...
						$cotizacion_content = '';
						$i = 1;
						if ( isset($cotizacion["cotizaciones"][$value[0]] ) ) {
							$conseptos = $cotizacion["cotizaciones"][$value[0]]["rowsData"];
						...
						if ($_permisos["adicionar"] == 1) { $cotizacion_content.= cotizacionFormContent($div, $i); }
							...
							<div class="panel panel-default panel-border-color ' . $_accordion_color . ' concepto" id="' . $div . '">
								...
								<div id="accordion_' . $div . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
									<div class="panel-body">
										<input type="hidden" id="id_concepto_' . $div . '" value="' . $value[0] . '">
		...
		case 'editar_adjunta_cotizacion':
			...
			if ($info["proveedores"]) {
				...
				if (isset($info["tramos"]["tramos"]["rowsData"])) {
					foreach ($info["tramos"]["tramos"]["rowsData"] as $key => $value) {
						$_origen.= '
							<tr>
								<td class="cell-detail">
				...
				foreach ($info["proveedores"] as $key => $value) {
					$arrayProveedores[$value[0]][] = $value;
					...
					';
					if ( isset($info["proveedor_cotizaciones"][ $value[0] ]) ) {
						$_cotizaciones = '
								...
								<thead>
									<tr>
										<th>
											<div class="be-checkbox">
												<center>
													<input type="checkbox" class="check_cotizacion_todos"id="check_cotizacion_todos_' . $value[0] . '">
													<label for="check_cotizacion_todos_' . $value[0] . '"></label>
												</center>
											</div>
										</th>
										<th>Concepto</th>
						...
						foreach ($info["proveedor_cotizaciones"][ $value[0] ] as $key_01 => $value_01) {
							$_id_cotizaciones.= $value_01[0] . ",";
							...
							$_cotizaciones.= '
								<tr>
									<td>
										<div class="be-checkbox">
											<center>
												<input type="checkbox" class="check_cotizacion" name="check_cotizacion_' . $value_01[0] . '" id="check_cotizacion_' . $value_01[0] . '" value="' . $value_01[0] . '" ' . $_disabled . '>
												<label for="check_cotizacion_' . $value_01[0] . '"></label>
											</center>
										</div>
									</td>
									<td class="cell-detail">
						...
						$_cotizaciones.= '
									<tr>
										<td colspan="5" class="text-center">
											...
											<input type="hidden" name="id_cotizaciones_' . $value[0] . '" value="' . $_id_cotizaciones . '">
										</td>
									</tr>
								</tbody>
							</table>
					...
					if ( isset($info["proveedor_facturas"][ $value[0] ]) ) {
						$_cant_facturas = COUNT($info["proveedor_facturas"][ $value[0] ]);
						$_facturas = '
							<strong>Facturas</strong>
							<table class="table table-hover">
								<thead>
						...
						foreach ($info["proveedor_facturas"][ $value[0] ] as $key_01 => $value_01) {
							if ($_flag_factura_valor_pesos AND $value_01["VALOR_PESOS"]) { $_valor_factura_pesos+= $value_01["VALOR_PESOS"]; }
							else { $_flag_factura_valor_pesos = false; }
							if ($_flag_factura_valor_usd AND $value_01["VALOR_USD"]) { $_valor_factura_usd+= $value_01["VALOR_USD"]; }
							else { $_flag_factura_valor_usd = false; }
					...
					if ($_cant_facturas > 0) {
						...
						if (isset($_valor_cotizacion_pesos) AND isset($_valor_factura_pesos)) {
							if ((int)$_valor_cotizacion_pesos == (int)$_valor_factura_pesos) { $_panel_color = "success"; }
							elseif ((int)$_valor_cotizacion_pesos < (int)$_valor_factura_pesos) { $_panel_color = "danger"; }
						}
					}
					...
					if ($_flag_muestra_formulario) {
						$_formulario = '
							<input type="hidden" name="id_proveedor_' . $value[0] . '" id="id_proveedor_' . $value[0] . '" value="' . $value[0] . '">
							<div class="form-group col-xs-12 col-sm-3 col-md-3">
								<label>(*) # Factura Proveedor</label>
								<input type="text" class="form-control input-sm" name="factura_' . $value[0] . '" id="factura_' . $value[0] . '" placeholder="# Factura">
							</div>
							<div class="form-group col-xs-12 col-sm-3 col-md-3">
								<label class="control-label">(*) Fecha Factura:</label>
								<div data-min-view="2" data-start-view="4" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1" class="input-group date datetimepicker" style="padding: 0">
									<input size="10" type="text" value="' . date('Y-m-d', $time) . '" name="fecha_' . $value[0] . '" id="fecha_' . $value[0] . '" readonly="" class="form-control input-sm" placeholder="Fecha Factura">
									<span class="input-group-addon btn btn-primary"><i class="icon-th mdi mdi-calendar"></i></span>
								</div>
							</div>
							<div class="form-group col-xs-12 col-sm-3 col-md-3">
								<label>(*) Valor Factura</label>
								<input type="text" class="form-control input-sm" name="valor_' . $value[0] . '" id="valor_' . $value[0] . '" onload="getFormatoNumeroDecimal(this)" onkeyup="getFormatoNumeroDecimal(this)" onchange="getFormatoNumeroDecimal(this)" placeholder="Valor Factura">
							</div>
							<div class="form-group col-xs-12 col-sm-3 col-md-3">
								<label>(*) Moneda Factura</label>
								' . $Internacional->getHtmlSelectMonedas( "moneda_" . $value[0] , "", "" )  . '
							</div>
							<div class="form-group col-xs-12 col-sm-12 col-md-12 text-center">
								<label>(*) Adjunto Factura</label>
								<input type="file" name="file_' . $value[0] . '" id="file_' . $value[0] . '" class="inputfile input-sm facturas" placeholder="Buscar Archivo...">
								<label for="file_' . $value[0] . '" class="btn-success input-xs"> 
									<i class="mdi mdi-upload"></i>
									<span>Buscar Archivo...</span>
								</label>
							</div>
					...
					$_msg_content["content"].= '
						<div class="panel panel-default panel-border-color panel-border-color-' . $_panel_color . ' proveedor" id="proveedor_' . $value[0] . '">
							<div class="panel-heading">
								<div class="tools">
								</div>
								<h4 class="panel-title">
									<a data-toggle="collapse" data-parent="#accordion2" href="#accordion_' . $value[0] . '" class="collapsed" aria-expanded="false"><i class="icon mdi mdi-chevron-down"></i> <span class="accordion_title">' . $value["proveedor"] . '</span>
				...
				foreach ($proveedores as $key => $value) {
					...
					if ( isset($cotizacion["proveedor_cotizaciones"][ $value[0] ]) ) {
						...
						foreach ($cotizacion["proveedor_cotizaciones"][ $value[0] ] as $key_01 => $value_01) {
							if ($_flag_cotizacion_valor_pesos AND $value_01["VALOR_PESOS"]) { $_valor_cotizacion_pesos+= $value_01["VALOR_PESOS"]; } 
							else { $_flag_cotizacion_valor_pesos = false; }

							if ($_flag_cotizacion_valor_usd AND $value_01["VALOR_USD"]) { $_valor_cotizacion_usd+= $value_01["VALOR_USD"]; } 
							else { $_flag_cotizacion_valor_usd = false;}
						}
		...
		case 'form_adjunta_egreso':
			...
			if ( $facturas ) {
				...
				if ( $facturas["proveedor"] ) {
					$proveedor = $facturas["proveedor"]["rowsData"][0];
				...
				if ( $facturas["facturas"] ) {
					...
					foreach ($facturas["facturas"]["rowsData"] as $key => $value) {
						...
						';
						if ($facturas["cotizaciones"][$value[0]]) {
							$_cant_conceptos = $facturas["cotizaciones"][$value[0]]["rowsNum"];
							...
							foreach ($facturas["cotizaciones"][$value[0]]["rowsData"] as $key_01 => $value_01) {
								...
								$_sobrecosto = "";
								if ($value_01["sobrecosto"]) { $_sobrecosto = '<strong class="text-danger">(Sobrecosto)</strong>'; }
						...
						$_facturas_content.= '
							<div class="panel panel-default panel-border-color panel-border-color-' . $_panel_color . ' proveedor">
								<div class="panel-heading">
									...
									<div class="tools">
										<div class="be-checkbox">
											<input class="check_facturas" value="' . $value[0] . '" id="check_factura_' . $value[0] . '" type="checkbox" data-moneda="' . $value["id_moneda"] . '" data-factura_pesos="' . $_valor_pesos . '" ' . $_check_disabled . '>
											<label for="check_factura_' . $value[0] . '"></label>
										</div>
									</div>
									<h4 class="panel-title">
										<a data-toggle="collapse" data-parent="#accordion1" href="#accordion_' . $value[0] . '" class="collapsed" aria-expanded="false"><i class="icon mdi mdi-chevron-down"></i> ' . $value["do"] . ' <small>Factura Proveedor: <strong>' . $value["num_factura"] . '</strong></small>
		...
		case 'ver_registro_egreso':
			...
			$proveedor = $anticipos["proveedor"]["rowsData"][0];
			...
			if ( $anticipos ) {
				...
				if ( $anticipos["facturas"] ) {
					...
					foreach ($anticipos["facturas"]["rowsData"] as $key => $value) {
						// Se valida se se puede seleccionar la factura para egreso 
						$_check_disabled = "disabled";
						$_panel_color = "danger";
						if ($value["TRM_FACTURA_PROVEEDOR"]) {
						...
						';
						if ($anticipos["cotizaciones"][$value[0]]) {
							...
							foreach ($anticipos["cotizaciones"][$value[0]]["rowsData"] as $key_01 => $value_01) {
								// Se verifica si la cotización corresponde a un sobrecosto
						...
						$_facturas_content.= '
							<div class="panel panel-default panel-border-color panel-border-color-' . $_panel_color . ' proveedor">
								<div class="panel-heading">
									...
									<h4 class="panel-title">
										<a data-toggle="collapse" data-parent="#accordion1" href="#accordion_' . $value[0] . '" class="collapsed" aria-expanded="false"><i class="icon mdi mdi-chevron-down"></i> ' . $value["do"] . ' <small><strong>' . $value["NOM_CLIENTE"] . '</strong></small>
		...
		case 'form_registro_material':
			...
			$general = $arrayDestinos["general"]["rowsData"][0];
			...
			$_form_content = '';
			$_tramos = '';
			foreach ($arrayDestinos["ciudades_destino"]["rowsData"] as $key => $value) {
				$panel_content = '
					<div id="accordion' . $value[0] . '" class="panel-group accordion">
				';
				foreach ($arrayDestinos["destinos"][$value[0]]["rowsData"] as $key_01 => $value_01) {
					$_tramos.= $value_01[0] . ',';
					$panel_content.= '
						<div class="panel panel-default">
							<div class="panel-heading">
								<h4 class="panel-title">
									<a data-toggle="collapse" data-parent="#accordion' . $value[0] . '" href="#collapse_' . $value_01[0] . '" class="collapsed" aria-expanded="false">
										<i class="icon mdi mdi-chevron-down"></i> ' . $value_01["sigla"] . '
										<span class="panel-subtitle">' . $value_01["direccion"] . '</span>
									</a>
								</h4>
							</div>
							<div id="collapse_' . $value_01[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
								<div class="panel-body">
									<input type="hidden" id="ciudad_destino_' . $value_01[0] . '" value="' . $value["municipio"] . ' (' . $value["depto"] . ' - ' . $value["pais"] . ')">
									<input type="hidden" id="destino_' . $value_01[0] . '" value="' . $value_01["sigla"] . '">
									<input type="hidden" class="tramos" name="id_tramo_' . $value_01[0] . '" id="id_tramo_' . $value_01[0] . '_1" value="' . $value_01[0] . '">
									<div class="form-group icon col-sm-12 div_registro_material_' . $value_01[0] . '">
										<button type="button" class="btn btn-space btn-success btn-small hint--top" data-hint="Agregar Material" id="btn_agrega_material_' . $value_01[0] . '"><span class="mdi mdi-plus"></span></button>
										<button type="button" class="btn btn-space btn-danger btn-small hint--top" data-hint="Quitar Material" id="btn_quita_material_' . $value_01[0] . '"><span class="mdi mdi-minus"></span></button>
									</div>
									<div class="registro_material_clon_' . $value_01[0] . '" id="form_registro_material_' . $value_01[0] . '_1">
										<div class="col-xs-12 col-sm-12 col-md-12" id="div_title_' . $value_01[0] . '_1">
											<div class="well well-sm">
												<h4 id="title_' . $value_01[0] . '_1">Material 1</h4>
											</div>
										</div>
										<div class="form-group col-xs-6 col-sm-3 col-md-3" id="div_delivery_' . $value_01[0] . '_1">
											<label class="control-label">Delivery:</label>
											<input type="text" name="delivery_' . $value_01[0] . '_1" id="delivery_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Delivery">
										</div>
										<div class="form-group col-xs-6 col-sm-3 col-md-3" id="div_shipment_' . $value_01[0] . '_1">
											<label class="control-label">Shipment:</label>
											<input type="text" name="shipment_' . $value_01[0] . '_1" id="shipment_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Shipment">
										</div>
										<div class="row"></div>
										<div class="form-group col-xs-12 col-sm-4 col-md-4" id="div_material_' . $value_01[0] . '_1">
											<label class="control-label">(*) Material:</label>
											<input type="text" name="material_' . $value_01[0] . '_1" id="material_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Material">
										</div>
										<div class="form-group col-xs-12 col-sm-3 col-md-3" id="div_posicion_arancelaria_' . $value_01[0] . '_1">
											<label class="control-label">Posición Arancelaria:</label>
											<input type="text" name="posicion_arancelaria_' . $value_01[0] . '_1" id="posicion_arancelaria_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Posición Arancelaria">
										</div>
										<div class="form-group col-xs-4 col-sm-2 col-md-2" id="div_cantidad_piezas_' . $value_01[0] . '_1">
											<label class="control-label">(*) # Piezas:</label>
											<input type="number" name="cantidad_piezas_' . $value_01[0] . '_1" id="cantidad_piezas_' . $value_01[0] . '_1" class="form-control input-sm" min="1" value="1">
										</div>
										<div class="form-group col-xs-8 col-sm-3 col-md-3" id="div_empaque_' . $value_01[0] . '_1">
											<label class="control-label">(*) Empaque:</label>
											' . $Internacional->getHtmlSelectEmpaques( "empaque_" . $value_01[0] . "_1" , "", "" ) . '
										</div>
										<div class="form-group col-xs-12 col-sm-3 col-md-3" id="div_peso_' . $value_01[0] . '_1">
											<label class="control-label">(*) Peso Unitario (Kg):</label>
											<input type="text" name="peso_' . $value_01[0] . '_1" id="peso_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Peso Unitario (Kg)" onkeyup="getFormatoNumeroDecimal(this)" onchange="getFormatoNumeroDecimal(this)" onload="getFormatoNumeroDecimal(this)">
										</div>
										<div class="form-group col-xs-4 col-sm-3 col-md-3" id="div_largo_' . $value_01[0] . '_1">
											<label class="control-label">(*) Largo Pieza (cm):</label>
											<input type="text" name="largo_' . $value_01[0] . '_1" id="largo_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Largo Pieza (cm)">
										</div>
										<div class="form-group col-xs-4 col-sm-3 col-md-3" id="div_alto_' . $value_01[0] . '_1">
											<label class="control-label">(*) Alto Pieza (cm):</label>
											<input type="text" name="alto_' . $value_01[0] . '_1" id="alto_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Alto Pieza (cm)">
										</div>
										<div class="form-group col-xs-4 col-sm-3 col-md-3" id="div_ancho_' . $value_01[0] . '_1">
											<label class="control-label">(*) Ancho Pieza (cm):</label>
											<input type="text" name="ancho_' . $value_01[0] . '_1" id="ancho_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="Ancho Pieza (cm)">
										</div>
										<div class="form-group col-xs-6 col-sm-2 col-md-2" id="div_check_refrigerado_' . $value_01[0] . '_1">
											<div class="be-checkbox">
												<input type="checkbox" name="check_refrigerado_' . $value_01[0] . '_1" id="check_refrigerado_' . $value_01[0] . '_1">
												<label for="check_refrigerado_' . $value_01[0] . '_1" id="label_check_refrigerado_' . $value_01[0] . '_1">Refrigerado</label>
											</div>
										</div>
										<div class="form-group col-xs-6 col-sm-3 col-md-3" id="div_check_peligroso_' . $value_01[0] . '_1">
											<div class="be-checkbox">
												<input type="checkbox" class="check_peligroso" name="check_peligroso_' . $value_01[0] . '_1" id="check_peligroso_' . $value_01[0] . '_1">
												<label for="check_peligroso_' . $value_01[0] . '_1" id="label_check_peligroso_' . $value_01[0] . '_1">Material Peligroso</label>
											</div>
										</div>
										<div class="form-group col-xs-6 col-sm-2 col-md-2 peligroso_content peligroso_content_' . $value_01[0] . '_1" id="div_un_' . $value_01[0] . '_1">
											<label class="control-label">(*) Código UN:</label>
											<input type="text" name="un_' . $value_01[0] . '_1" id="un_' . $value_01[0] . '_1" class="form-control input-sm" placeholder="UN" maxlength="4">
										</div>
										<div class="form-group col-xs-6 col-sm-2 col-md-2 peligroso_content peligroso_content_' . $value_01[0] . '_1" id="div_riesgo_' . $value_01[0] . '_1">
											<label class="control-label">(*) Riesgo:</label>
											' . $Internacional->getHtmlSelectRiesgoMaterial( "riesgo_" . $value_01[0] . "_1" , "", "" ) . '
										</div>
										<div class="form-group col-xs-6 col-sm-3 col-md-3 peligroso_content peligroso_content_' . $value_01[0] . '_1" id="div_hoja_seguridad_file_' . $value_01[0] . '_1">
											<label class="control-label">(*) Hoja de Seguridad:</label><br>
											<input type="file" name="hoja_seguridad_file_' . $value_01[0] . '_1" id="hoja_seguridad_file_' . $value_01[0] . '_1" class="inputfile">
											<label for="hoja_seguridad_file_' . $value_01[0] . '_1"  id="label_hoja_seguridad_file_' . $value_01[0] . '_1" class="btn-success"> <i class="mdi mdi-upload"></i><span>Buscar Archivo...</span>:</label>
										</div>
										<div class="row"></div>
									</div>
									<div class="row"></div>
				...
				$_form_content.= '
					<div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a data-toggle="collapse" data-parent="#accordion1" href="#collapse_' . $value[0] . '">
									<i class="icon mdi mdi-chevron-down"></i> ' . $value["municipio"] . ' <small>' . $value["depto"] . ' - ' . $value["pais"] . '</small>
									<span class="panel-subtitle">Destinos: <strong>' . $arrayDestinos["destinos"][$value[0]]["rowsNum"] . '</strong></span>
								</a>
							</h4>
						</div>
						<div id="collapse_' . $value[0] . '" class="panel-collapse collapse">
							<div class="panel-body">
								' . $panel_content . ' 
							</div>
						</div>
		...
		case 'form_registro_documentos':
			...
			$general = $arrayDocumentos["general"]["rowsData"][0];
			...
			foreach ($arrayDocumentos["tipo_documentos"]["rowsData"] as $key => $value) {
				$_panel_color = 'default';
				$_fecha_gestion =  '';
					...
					$_fecha_gestion = '
						<div class="col-xs-12 col-sm-4 col-md-3">
							...
							<div data-min-view="2" data-start-view="4" data-date="" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1" class="input-group date datetimepicker">
								<input size="16" type="text" value="" id="fecha_gestion_' . $value[0] . '" name="fecha_gestion_' . $value[0] . '" class="form-control input-sm fecha_gestion" readonly="”readonly”"><span class="input-group-addon btn btn-primary"><i class="icon-th mdi mdi-calendar"></i></span>
							</div>
						</div>
					';
				}
				...
				$_panel_content = '
					<input type="hidden" id="documento_' . $value[0] . '" value="' . $value["nombre"] . '">
					' . $_fecha_gestion . '
					<label class="control-label">Documento Adjunto:</label><br>
					<input type="file" name="documento_file_' . $value[0] . '" id="documento_file_' . $value[0] . '" class="inputfile file_documento">
					<label for="documento_file_' . $value[0] . '" class="btn-success"> <i class="mdi mdi-upload"></i><span>Buscar Archivo...</span>:</label>
					<div class="be-checkbox">
						<input name="soporte_' . $value[0] . '" id="soporte_' . $value[0] . '" type="checkbox">
						<label for="soporte_' . $value[0] . '">Soporte Facturación</label>
					</div>
				';

				$_soporte_facturacion = '';
				if ( isset($_arrayDocumentos[$value[0]]) ) {
					$documento_envio = $_arrayDocumentos[$value[0]];
				...
				$documentos_content.= '
					<div class="panel panel-default panel-border-color panel-border-color-' . $_panel_color . '">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a data-toggle="collapse" data-parent="#accordion_form_documentos" href="#collapse_documentos_' . $value[0] . '" class="collapsed" aria-expanded="false">
						...
						<div id="collapse_documentos_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
							<div class="panel-body">
		...
		case 'form_gestiona_actividad_documentos':
			...
			$general = $documentos["general"]["rowsData"][0];
		...
		case 'form_instruccion_factura':
			...
			if ( $instruccion["proyectos"] ) {
				foreach ($instruccion["proyectos"]["rowsData"] as $key => $value) {
					...
					$_tipo_transporte = "";
					if ($value["tipo_transporte"]) { $_tipo_transporte = '(' . $value["tipo_transporte"] . ')'; }
					...
					if ( $instruccion["tramos"][$value["ID_PROYECTO_INTERNACIONAL"]] ) {
						...
						if ($arrayTramos) {
							...
							foreach ($arrayTramos["rowsData"] as $key_tramos => $value_tramos) {
					...
					if ( $_flag_valor_pesos ) {
						...
						$_check_agenciamiento = '
							<div class="be-checkbox %visible%">
								<input name="agenciamiento_' . $value[0] . '" class="check_agenciamiento" id="check_agenciamiento_' . $value[0] . '" type="checkbox">
								<label for="check_agenciamiento_' . $value[0] . '">Agenciamiento</label>
							</div>
							<input type="hidden" class="valor_pesos_fijo" value="' . (float)$value["VALOR_PESOS_FACTURA"] . '">
							<input type="hidden" class="valor_pesos" name="valor_pesos_' . $value[0] . '" value="' . (float)$value["VALOR_PESOS_FACTURA"] . '">
							<input type="hidden" class="valor_agenciamiento" name="valor_agenciamiento_' . $value[0] . '">
						...
						if ( $instruccion["documentos_envio"][$value[0]] ) {
							...
							$_tabla_documentos = '
								<table id="table1" class="table table-striped table-hover table-fw-widget" data-page-length="50">
									...
									<tbody>
							';
							foreach ($instruccion["documentos_envio"][$value[0]] as $key_01 => $value_01) {
								$_check_seleccionado = '';
								...
								$_tabla_documentos.= '
									<tr>
										<td>
											<div class="be-checkbox">
												<center>
													<input class="check_documento" value="' . $value_01[0] . '" id="check_documento_' . $value_01[0] . '" type="checkbox" onclick="cambiaEntregableDocumento(this)" ' . $_check_seleccionado . '>
													<label for="check_documento_' . $value_01[0] . '"></label>
												</center>
											</div>
										</td>
										<td class="cell-detail">
							...
							$_documentos_accordion = '
								<div class="panel panel-default panel-border-color panel-border-color-default documentos">
									<div class="panel-heading">
										<div class="tools"></div>
										<h4 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion_instruccion_' . $value[0] . '" href="#accordion_documentos_' . $value[0] . '" class="collapsed" aria-expanded="false">
									...
									<div id="accordion_documentos_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
										<div class="panel-body">
						...
						if ( isset($instruccion["impuestos"][$value[0]]) ) {
							...
							foreach ($instruccion["impuestos"][$value[0]] as $key_01 => $value_01) {
								...
								if ($value_01["VALOR_PESOS_FACTURA"]) {
									$_valor_impuesto_pesos = '
										...
										<input type="hidden" class="datos_impuesto" data-descripcion="' . $value_01["nombre"] . ' - ' . $value_01["descripcion"] . '" data-valor_pesos="' . $value_01["VALOR_PESOS_FACTURA"] . '" value="' . (float)$value_01[0] . '">
										<input type="hidden" class="valor_impuesto" name="valor_impuesto_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["valor"] . '">
										<input type="hidden" class="valor_impuesto_pesos" name="valor_impuesto_pesos_' . $value[0] . '_' . $value_01["ID_COTIZACION"] . '" value="' . (float)$value_01["VALOR_PESOS_FACTURA"] . '">
										<input type="hidden" class="id_moneda_impuesto" name="id_moneda_impuesto_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["id_moneda"] . '"
										>
										<input type="hidden" class="moneda_impuesto" name="moneda_impuesto_' . $value[0] . '_' . $value_01[0] . '" value="' . $value_01["MONEDA"] . '" data-id_moneda_impuesto="' . (float)$value_01["id_moneda"] . '" data-valor_impuesto="' . (float)$value_01["valor"] . '" data-valor_impuesto_pesos="' . (float)$value_01["VALOR_PESOS_FACTURA"] . '">
									';
								} else {
									$_flag_habilita_check_impuestos = false;
								}
								...
								if ($value_01["VALOR_USD_FACTURA"]) {
									$_valor_impuesto_usd = '
										...
										<input type="hidden" name="valor_impuesto_usd_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["VALOR_USD_FACTURA"] . '">
									';
								}
							...
							$_impuestos_accordion = '
								<div class="panel panel-default panel-border-color panel-border-color-default impuestos">
									<div class="panel-heading">
										<div class="tools"></div>
										<h4 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion_instruccion_' . $value[0] . '" href="#accordion_impuestos_' . $value[0] . '" class="collapsed" aria-expanded="false">
												<i class="icon mdi mdi-chevron-down"></i> Impuestos
												<span class="panel-subtitle">Lista de impuestos generados en el poryecto</span>
											</a>
										</h4>
									</div>
									<div id="accordion_impuestos_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
										<div class="panel-body">
											<div class="col-xs-12 col-sm-12 col-md-12">
												<input type="hidden" name="id_impuestos_' . $value[0] . '" value="' . $_id_cotizacion_impuestos . '">
												' . $_suma_impuestos . '
												' . $_tabla_impuestos . '
											</div>
										</div>
						...
						if ( isset($instruccion["conceptos"][$value[0]][1] ) ) {
							...
							$_tabla_conceptos = '
								<table id="table1" class="table table-striped table-hover table-fw-widget" data-page-length="50">
									...
									<tbody>
							';
							foreach ($instruccion["conceptos"][$value[0]][1] as $key_01 => $value_01) {
								$_check_seleccionado = '';
								...
								$_tabla_conceptos.= '
									<tr>
										<td>
											<div class="be-checkbox">
												<center>
													<input class="check_concepto" data-concepto="' . $value_01["nombre"] . '" value="' . $value_01[0] . '" id="check_concepto_' . $value_01[0] . '" type="checkbox" onclick="cambiaVistaConcepto(this)" ' . $_check_seleccionado . '>
													<label for="check_concepto_' . $value_01[0] . '"></label>
												</center>
											</div>
										</td>
										<td class="cell-detail">
							...
							$_conceptos_accordion = '
								<div class="panel panel-default panel-border-color panel-border-color-default conceptos">
									<div class="panel-heading">
										<div class="tools"></div>
										<h4 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion_instruccion_' . $value[0] . '" href="#accordion_conceptos_' . $value[0] . '" class="collapsed" aria-expanded="false">
									...
									<div id="accordion_conceptos_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
										<div class="panel-body">
											<div class="col-xs-1 col-sm-1 col-md-2"></div>
											<div class="col-xs-12 col-sm-12 col-md-8">
												' . $_tabla_conceptos . '
						...
						if ( isset($instruccion["sobrecostos"][$value[0]]) ) {
							$_tabla_sobrecostos = '
								<table id="table1" class="table table-striped table-hover table-fw-widget" data-page-length="50">
									<thead>
										<tr>
							...
							$_total_sobrecostos = 0;
							$_id_sobrecostos = "";
							foreach ($instruccion["sobrecostos"][$value[0]] as $key_01 => $value_01) {
								...
								if ($value_01["VALOR_PESOS_FACTURA"]) {
									$_valor_sobrecosto_pesos = '
										<span>' . number_format($value_01["VALOR_PESOS_FACTURA"], 2, ',', '.') . '</span>
										<input type="hidden" class="valor_sobrecosto" name="valor_sobrecosto_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["valor"] . '">
										<input type="hidden" class="valor_sobrecosto_pesos" name="valor_sobrecosto_pesos_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["VALOR_PESOS_FACTURA"] . '">
										<input type="hidden" class="id_moneda_sobrecosto" name="id_moneda_sobrecosto_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["id_moneda"] . '">
										<input type="hidden" class="moneda_sobrecosto" name="moneda_sobrecosto_' . $value[0] . '_' . $value_01[0] . '" value="' . $value_01["MONEDA"] . '">
									';
								}
								...
								if ($value_01["VALOR_USD_FACTURA"]) {
									$_valor_sobrecosto_usd = '
										<span>' . number_format($value_01["VALOR_USD_FACTURA"], 2, ',', '.') . '</span>
										<input type="hidden" name="valor_sobrecosto_usd_' . $value[0] . '_' . $value_01[0] . '" value="' . (float)$value_01["VALOR_USD_FACTURA"] . '">
									';
								}
								...
								$_tabla_sobrecostos.= '
									<tr>
										...
										<td class="cell-detail">
											<input type="text" class="form-control input-xs valor_facturar" name="valor_facturar_' . $value[0] . '_' . $value_01["ID_COTIZACION"] . '" value="' . number_format($value_01["VALOR_PESOS_FACTURA"], 2, ',', '.') . '" onload="getFormatoNumeroDecimal(this)" onkeyup="getFormatoNumeroDecimal(this)" onchange="getFormatoNumeroDecimal(this)" placeholder="Valor a Facturar" data-descripcion="' . $value_01["nombre"] . ' - ' . $value_01["descripcion"] . '" data-id="' . (float)$value_01[0] . '">
										</td>
										<td class="actions">
											<a href="' . BASE_URL . 'public/files/internacional/facturas/' . $value_01["id_intr_proyecto"] . '/' . $value_01["id_proveedor"] . '/' . $value_01["url"] . '" target="_blank" class="cell-detail hint--top-left" data-hint="Descargar Impuesto"><span class="icon mdi mdi-download"></span></a>
										</td>
							...
							$_sobrecostos_accordion = '
								<div class="panel panel-default panel-border-color panel-border-color-default sobrecostos">
									<div class="panel-heading">
										<div class="tools"></div>
										<h4 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion_instruccion_' . $value[0] . '" href="#accordion_sobrecostos_' . $value[0] . '" class="collapsed" aria-expanded="false">
									...
									<div id="accordion_sobrecostos_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
										<div class="panel-body">
											<div class="col-xs-12 col-sm-12 col-md-12">
												<input type="hidden" class="total_sobrecosto" name="total_sobrecosto_' . $value[0] . '" value="' . number_format($_total_sobrecostos,0,"","")  . '">
												<input type="hidden" name="id_sobercosto_' . $value[0] . '" value="' . $_id_sobrecostos  . '">
												' . $_tabla_sobrecostos . '
											</div>
										</div>
									</div>
							...
							$_ajuste_oferta_accordion = '
								<div class="panel panel-default panel-border-color panel-border-color-warning ajuste_oferta" data-valor_oferta="' . $value["VALOR_OFERTA"] . '" data-fecha_oferta="' . $value["FECHA_OFERTA"] . '" data-id_oferta="' . $value["ID_OFERTA"] . '">
									<div class="panel-heading">
										<div class="tools"></div>
										<h4 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion_instruccion_' . $value[0] . '" href="#accordion_ajuste_oferta_' . $value[0] . '" class="collapsed" aria-expanded="false">
									...
									<div id="accordion_ajuste_oferta_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
										<div class="panel-body">
						...
						if ( $_flag_documentos OR $_flag_impuestos OR $_flag_conceptos ) {
							$_proyecto_accordion = '
								<div id="accordion_instruccion_' . $value[0] . '" class="panel-group accordion">
									' . $_ajuste_oferta_accordion . '
									' . $_sobrecostos_accordion . '
									' . $_impuestos_accordion . '
									' . $_documentos_accordion . '
						...
						if (!$_flag_habilita_check_impuestos) { $_flag_agenciamiento_visible = 'agenciamiento_visible'; }
					...
					$_habilita_check = '';
					if (!$_flag_habilita_check_proyecto OR !$_flag_habilita_check_impuestos) { $_habilita_check = 'disabled'; }

					if (isset($instruccion["impuestos"][$value[0]]) AND !$_flag_habilita_check_impuestos) { $_flag_form_content = ''; }

					$_accordion_content.= '
						<div class="panel panel-default panel-border-color panel-border-color-default" data-id_proyecto="' . $value[0] . '">
							<div class="panel-heading">
								<div class="tools">
									<a href="' . BASE_URL . 'public/files/internacional/oferta_comercial/' . $value[0] . '/' . $value["url"] . '" target="_blank" class="cell-detail hint--top-left" data-hint="Descargar Oferta Comercial">
										<span class="icon mdi mdi-download"></span>
									</a>
								</div>
								<div class="tools">
									<div class="be-checkbox">
										<input class="check_instrucciones" value="' . $value[0] . '" id="check_instruccion_' . $value[0] . '" type="checkbox" ' . $_habilita_check . '>
										<label for="check_instruccion_' . $value[0] . '"></label>
									</div>
								</div>
								<h4 class="panel-title">
									<a data-toggle="collapse" data-parent="#accordion1" href="#accordion_' . $value[0] . '" class="collapsed" aria-expanded="false"><i class="icon mdi mdi-chevron-down"></i> ' . $value["do"] . ' <small>' . $value["importacion"] . '</small>
				...
				if ( isset($info["tramos"]["tramos"]["rowsData"]) ) {
					...
					foreach ($info["tramos"]["tramos"]["rowsData"] as $key_01 => $value_01) {
						if ( $value_01["tipo_tramo"] == "Cargue") {
							...
							$_deliveries = '';
							if ( isset($info["tramos"]["material"][$value_01[0]][0]["rowsData"] ) ) {
								$_flag_varios_delivery = false;
								$_delivery_content = '';
								foreach ( $info["tramos"]["material"][$value_01[0]][0]["rowsData"] as $key_02 => $value_02 ) {
		...
		case 'ajusta_documento_entrega':
			...
			$array = Array();
			$array["soporte_facturacion"] = (boolean)$_POST["soporte_facturacion"];
		...
		case 'ajusta_vista_concepto':
			...
			$array = Array();
			$array["fct_muestra_concepto"] = (boolean)$_POST["fct_muestra_concepto"];
			$Data->updateRegistro( "cmx_intr_cotizaciones", $array, (int)$_POST["id"] );
			break;
		...
		case 'form_editar_factura':
			...
			/***** Contenido de la información del cliente *****/
			$cliente = $info_factura["cliente"]["rowsData"][0];
			$_info_cliente = datosClienteEncabezado( $cliente );
			...
			if ( $info_factura["proyectos"] ) {
				...
				if ( $info_factura["cliente"] ) {
					$encabezado = $info_factura["cliente"]["rowsData"][0];
					$_encabezado = '
						<table class="table table-condensed table-bordered">
							<tbody>
								<tr>
				...
				if ($info_factura["proyectos"]) {
					...
					$_impuestos = '';
					foreach ($info_factura["proyectos"]["rowsData"] as $key => $value) {
						$id_intr_proyecto = $value[0];
						...
						$_tramo_destino = '';
						$_guias = '';
						if ( isset($info_factura["tramos"][$value[0]]["tramos"]["rowsData"]) ) {
							$_tramo_origen = ' | Origen: [ ';
							$_flag_varios_origen = false;
							$_tramo_destino = ' | Destino: [ ';
							$_flag_varios_destino = false;
							$_guias = ' | Guías: [ ';
							$_flag_guias = false;
							foreach ($info_factura["tramos"][$value[0]]["tramos"]["rowsData"] as $key_01 => $value_01) {
								if ( $value_01["tipo_tramo"] == "Cargue") {
									if ($_flag_varios_origen) {
										$_tramo_origen.= ' | ' . $value_01["municipio"] . ' ';
									} else {
									...
									// // Se buscan los deliveries de la guía 
									$_deliveries = '';
									if ( isset($info_factura["tramos"][$value[0]]["material"][$value_01[0]][0]["rowsData"] ) ) {
										$_flag_varios_delivery = false;
										$_delivery_content = '';
										foreach ( $info_factura["tramos"][$value[0]]["material"][$value_01[0]][0]["rowsData"] as $key_02 => $value_02 ) {
											if ($value_02["delivery"]) {
												if ( $_flag_varios_delivery ) {
													$_delivery_content.= ' | ' . $value_02["delivery"] . ' ';
												} else {
						...
						// Se pregunta si se debe mostrar conceptos del proyecto
						$_conceptos = '';
						if ( isset($info_factura["conceptos"][$value[0]][1]) ) {
							$_flag_varios_conceptos = false;
							foreach ($info_factura["conceptos"][$value[0]][1] as $key_01 => $value_01) {
						...
						if ( isset($info_factura["impuestos"][$value[0]]) ) {
							$_impuestos.= '[ <strong>' . $_do . '</strong> ] Impuestos: [ ';
							$_flag_varios_impuestos = false;
							foreach ($info_factura["impuestos"][$value[0]] as $key_01 => $value_01) {
								if ($_flag_varios_impuestos) {
									$_impuestos.= ' | ' . number_format($value_01["valor"], 2, ',', '.') . ' ' . $value_01["MONEDA"];
								} else {
									$_impuestos.= ' ' . number_format($value_01["valor"], 2, ',', '.') . ' ' . $value_01["MONEDA"];
						...
						// Se agrega los impuestos incurridos en el proyecto
						if (isset( $info_factura["impuestos"][$value[0]] ) AND $info_factura["impuestos"][$value[0]]) {
							foreach ($info_factura["impuestos"][$value[0]] as $key_01 => $value_01) {
								$_cuerpo.= '
									<div class="col-xs-12 col-sm-3 col-md-2 text-center">
										<span></span>
						...
						// Se agrega los sobrecostos incurridos en el proyecto
						if (isset( $info_factura["sobrecostos"][$value[0]] ) AND $info_factura["sobrecostos"][$value[0]]) {
							foreach ($info_factura["sobrecostos"][$value[0]] as $key_01 => $value_01) {
								$_cuerpo.= '
									<div class="col-xs-12 col-sm-3 col-md-2 text-center">
										<span></span>
									</div>
		...
		case 'form_pago_cartera':
			...
			if ($info_factura["general"]) {
				...
				foreach ($info_factura["general"] as $key => $value) {
					$_do = "";
					if ( $info_factura["do"][$value[0]] ) {
						foreach ($info_factura["do"][$value[0]] as $key_01 => $value_01) {
					...
					$table_content.='
						<tr>
							<td>
								<div class="be-checkbox">
									<input type="checkbox" class="check_factura" id="check_factura_' . $value[0] . '" data-id_factura="' . $value[0] . '" data-total="' . $value["total"] . '">
									<label for="check_factura_' . $value[0] . '"></label>
								</div>
							</td>
							<td class="cell-detail">
							...
							<td class="actions">
								<a href="javascript:" onclick="verDatosCartera(' . $value[0] . ')" class="cell-detail hint--top-left" data-hint="Ver Factura"><span class="icon mdi mdi-eye" data-toggle="modal" data-target="#ver_factura"></span></a>
							</td>
						</tr>
					';
				}
			...
			if (isset($_FILES["url"]) AND $_FILES["url"]) {
				...
				if ( $file["error"] == 0) {
					...
					if ($id_pago) {
						...
						';
						if (!$Data->ejecuteRegistro($sql)) { $_msg_error.= "<p>Error al actualizar el pago en las facturas.</p>"; } 
		...
		case 'table_cartera_pagada':
			...
			if ( $arrayCartera["general"] ) {
				foreach ($arrayCartera["general"] as $key => $value) {
					$total_pagado = 0;
					if ($arrayCartera["pagado"][$value[0]]) { 
						$total_pagado = $arrayCartera["pagado"][$value[0]]; 
					}
					...
					$table_content.='
						<tr>
							...
							<td class="actions">
								<a href="javascript:" onclick="verPagosRegistrados(' . $value[0] . ')" class="cell-detail hint--top-left" data-hint="Ver Pagos"><span class="icon mdi mdi-eye" data-toggle="modal" data-target="#ver_cartera"></span></a>
							</td>
						</tr>
					';
				}
		...
		case 'form_cartera_pagada':
			...
			if ($info_factura["general"]) {
				...
				foreach ($info_factura["general"] as $key => $value) {
					...
					if ($info_factura["facturas"][$value[0]]) {
						foreach ($info_factura["facturas"][$value[0]] as $key_01 => $value_01) {
							$_content_dos = '
								<div role="alert" class="alert alert-warning alert-icon alert-icon-border alert-dismissible">
									<div class="icon"><span class="mdi mdi-close"></span></div>
									<div class="message">
							...
							if ($info_factura["do"][$value[0]][$value_01[0]]) {
								$_content_dos = '
									<div class="table-responsive noSwipe col-xs-12 col-sm-12 col-md-12">
										<table id="table" class="table table-hover table-condensed">
											...
											<tbody>
								';
								foreach ($info_factura["do"][$value[0]][$value_01[0]] as $key_02 => $value_02) {
									$_content_dos.= '
										<tr>
											<td class="cell-detail">
							...
							$_accordion_content_facturas.= '
								<div class="panel panel-default panel-border-color panel-border-color-default">
									<div class="panel-heading">
										...
										<h4 class="panel-title">
											<a data-toggle="collapse" data-parent="#accordion2" href="#accordion_facturas_' . $value[0] . '_' . $value_01[0] . '" class="collapsed" aria-expanded="false"><i class="icon mdi mdi-chevron-down"></i> 
									...
									<div id="accordion_facturas_' . $value[0] . '_' . $value_01[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
					...
					if ($value["url"]) {
						$_btn_pago = '
							<a href="' . BASE_URL . 'public/files/internacional/recibo_caja/' . $value[0] . '/' . $value["url"] . '" target="_blank" class="cell-detail hint--top-left" data-hint="Descargar Recibo de Caja">
		 						<span class="icon mdi mdi-download"></span>
		 					</a>
						';
					}
					...
					$_accordion_content.= '
						<div class="panel panel-default panel-border-color panel-border-color-default">
							<div class="panel-heading">
								...
								<h4 class="panel-title">
									<a data-toggle="collapse" data-parent="#accordion1" href="#accordion_pago_' . $value[0] . '" class="collapsed" aria-expanded="false"><i class="icon mdi mdi-chevron-down"></i> ' . $value["recibo_caja"] . ' <small><strong>' . $value["numero_pago"] . '</strong> | Fecha Pagado: <strong>' . $value["fecha_pago"] . '</strong></small>
										<span class="panel-subtitle">
							...
							<div id="accordion_pago_' . $value[0] . '" class="panel-collapse collapse" aria-expanded="false" style="height: 0px;">
	...
	function cotizacionesContent($array, $div, $id_perfil){
		...
		foreach ($array as $key_01 => $value_01) {
			...
			$_check_autoriza = '';
			if ( $id_perfil == 13 OR $id_perfil == 1 OR $id_perfil == 25 ) {
				$_check_autoriza = '
			...
			$cotizacion_content.= '
				<div class="col-xs-12 col-sm-3 col-md-3 panel-body panel-body-contrast div_' . $div . '" id="div_' . $div . '_' . $i . '">
					<div class="row">
						...
						<div class="col-xs-12">
							<input type="text" class="form-control input-xs" placeholder="Valor Cotización" value="' . number_format($value_01["valor"], 2, ',', '.') . '" readonly>
						</div>
						<div class="col-xs-12">
						...
						<div class="col-xs-12">
							<textarea class="form-control input-xs" placeholder="Descripción" readonly>' . $value_01["descripcion"] . '</textarea>
						</div>
						' . $_check_autoriza . '
					</div>
				</div>
			...
			<div class="col-xs-12 col-sm-3 col-md-3 panel-body panel-body-contrast">
				<div class="row">
					<form id="form_' . $div . '">
						...
						<div class="col-xs-12">
							<div data-min-view="2" data-start-view="4" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1" class="input-group date datetimepicker" style="padding: 0">
								<input size="10" type="text" value="'.date('Y-m-d').'" name="' . $div . '_fecha" id="' . $div . '_fecha" readonly="" class="form-control input-xs" placeholder="Fecha Cotización" disabled="disabled">
								<span class="input-group-addon btn btn-primary"><i class="icon-th mdi mdi-calendar"></i></span>
							</div> 
		...
		if ($array) {
			...
			foreach ($array as $key => $value) {
				...
				foreach ($materiales as $key_01 => $value_01) {
					...
					$_un = '
						<input type="text" name="un_' . $value_01[0] . '" id="un_' . $value_01[0] . '" class="form-control input-xs" placeholder="UN" maxlength="4">
					';

					$_riesgo = $Internacional->getHtmlSelectRiesgoMaterial_xs( "riesgo_" . $value_01[0], "", "" );

					$_hoja_seguridad = '
						<td class="cell-detail text-center">
							<input type="file" name="hoja_seguridad_file_' . $value_01[0] . '" id="hoja_seguridad_file_' . $value_01[0] . '" class="inputfile">
							<label for="hoja_seguridad_file_' . $value_01[0] . '"  id="label_hoja_seguridad_file_' . $value_01[0] . '" class="btn-success"><i class="mdi mdi-upload"></i></label>
						</td>
					';
					...
					$materiales_content.= '
						<tr>
							...
							<td>
								<div class="be-checkbox">
									<input id="check_refrigerado_' . $value_01[0] . '" class="check_refrigerado" type="checkbox" ' . $_refrigerado_checked . ' ' . $_refrigerado_checked_disabled . '>
									<label for="check_refrigerado_' . $value_01[0] . '"></label>
								</div>
							</td>
							<td class="cell-detail text-center">
								<input type="hidden" id="guia_' . $value_01[0] . '" value="' . $encabezado["guia"] . '">
								<input type="hidden" id="material_' . $value_01[0] . '" value="' . $value_01["material"] . '">
								<input type="hidden" name="tramo_' . $value_01[0] . '" value="' . $value_01["id_tramo"] . '">
								<div class="be-checkbox">
									<input id="check_peligroso_' . $value_01[0] . '" class="check_peligroso" type="checkbox" ' . $_peligroso_checked . ' ' . $_peligroso_checked_disabled . '>
									<label for="check_peligroso_' . $value_01[0] . '"></label>
								</div>
							</td>
							<td class="cell-detail text-center">
								' . $_un . '
				...
				$guias.= '
					<div class="panel panel-default">
						<div class="panel-heading">
							<h4 class="panel-title">
								<a data-toggle="collapse" data-parent="#accordion_form_material" href="#collapse_' . $encabezado[0] . '" class="collapsed" aria-expanded="false">
									<i class="icon mdi mdi-chevron-down"></i> ' . $encabezado["guia"] . ' <small>' . $encabezado["tipo_transporte"] . '</small>
									<span class="panel-subtitle"><strong>' . $encabezado["sigla"] . '</strong> ' . $encabezado["direccion"] . ' <strong>' . $encabezado["municipio"] . ' (' . $encabezado["depto"] . ' - ' . $encabezado["pais"] . ')</strong></span>
								</a>
							</h4>
