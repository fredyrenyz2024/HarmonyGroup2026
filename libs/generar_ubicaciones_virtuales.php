<?php 
	...
	$dir_pasillo = $dir_bodega . '0/';
	//Si no existe la carpeta la creamos
	if (!file_exists($dir_pasillo)){
		mkdir($dir_pasillo);
	}

	for ($j=1; $j <= $_POST["ubicaciones_virtuales"] ; $j++) { 
		//carpeta de columna
		...
		$codigo_bodega =	$_POST["id_bodega"] . "-" . $linea . "-" . pasaLetra($j) . "-" . $nivel;
		// print_r("\n" . $codigo_bodega);
		...
		$arrayPosicion["linea"] = $linea;
		$arrayPosicion["columna"] = pasaLetra($j);
		$arrayPosicion["nivel"] = $nivel;
		$arrayPosicion["tipo_ubicacion"] = "Saldos";
