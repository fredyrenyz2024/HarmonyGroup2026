...
switch ($_REQUEST['action']) {
    ...
    case 'num_cotizacion':
        ...
        $sql = "SELECT max(n_cotizacion)+1 AS nco
    ...
    case 'Registrar_Destinatarios':
        ...
        for ($z = 0; $z < $nFilas; $z++) {
			...
			VALUES(null,'$numero_solicita','$ciu','$direcci','$desti','$fech','$observacion','$fecha','$hora','$user','$horades','punto entrega','0','$tel','$peso','$lugar','$idpunto','$estado_destinatario')";
    ...
    case 'consultar_solicitud_servicio':
        ...
        $sql = "SELECT se.*,
			...
			INNER JOIN cmx_municipios a

			ON se.origen=a.rndc_codigo_ciudad

			INNER JOIN cmx_municipios b

			ON se.destino=b.rndc_codigo_ciudad

			INNER JOIN cmx_clientes cli

			ON se.nombre_cliente=cli.nombre

			WHERE se.id=" . $idsolicitud . "";
        ...
        if ($result2) {
            ...
            /*$sqldest="SELECT a.solicitud_servicio, a.direccion_entrega,

            a.fecha_estimada_entrega, a.observacion, a.hora_estimada,

            a.telefono, a.peso, a.id_punto, mn.municipio
            ...
            WHERE solicitud_servicio=".$idsolicitud;

            $resultdes= $Data->getConsulta($sqldest);*/
            ...
            /*foreach($resultdes["rowsData"] as $index => $element){

        if(strcasecmp($element[''], $resultdes["rowsData"][0]['id_punto'])==0){

        $array_destinata[$index]['selected']=true;
        ...
        $array_destinata[$index]['destina_desti']= $element[''];

        $array_destinata[$index]['direcci_desti']= $element[''];

        $array_destinata[$index]['telefono_desti']= $element[''];

        $array_destinata[$index]['peso_desti']= $element[''];
    ...
    case 'tabla_solicitudes':
        ...
        b.item, b.tipo_carga, b.tipo_transporte,
        ...
        LEFT JOIN cmx_solicitud_vehiculo2 AS soli

        ON b.id=soli.idpareja_origen_destino AND soli.estado=
    ...
    case 'consulte_movimientos':
        ...
        $sql = '
				...
				SELECT s.n_cotizacion, s.fecha, s.hora,s.estado,s.usuario_auditor,s.proceso,

				 m.n_cotizacion, m.id AS pareja,m.item
        ...
        $sql2 = 'SELECT a.n_cotizacion, a.fecha, a.hora, a.usuario_auditor,b.estado
    ...
    case 'traer_flete':
        ...
        //CONSULTA EL CODIGO DEL MUNICIPIO DE ORIGEN

        /*$sql="SELECT id FROM cmx_municipios

        WHERE rndc_codigo_ciudad=".$ori." ";

        $resultt = $Data->getConsulta($sql);

        $idorigen=$resultt["rowsData"][0]["id"];

        //CONSULTA EL CODIGO DEL MUNICIPIO DE DESTINO

        $sql2="SELECT id FROM cmx_municipios

        WHERE rndc_codigo_ciudad=".$des."  ";

        $resultd = $Data->getConsulta($sql2);

        $iddestino=$resultd["rowsData"][0]["id"];

        //CONSULTA EL CODIGO DEL VEHICULO

        $sql3="

        SELECT id FROM cmx_para_tipo_vehiculo

        WHERE nombre='".$vehiculo."'

        ";

        // echo $sql3;

        $resultv = $Data->getConsulta($sql3);

        $idvehiculo=$resultv["rowsData"][0]["id"];

        //CONSULTA DEL FLETE

        $sql4="SELECT * FROM cmx_fletes_nacional

        WHERE origen='".$idorigen."'

        and destino='".$iddestino."'

        and  tipo_vehiculo='".$idvehiculo."'
    ...
    case 'historico_cotizacion':
        ...
        $sql = '
				...
				SELECT e.*, m.id AS pareja, m.item, m.proceso, m.tipo_mercancia

				FROM cmx_estados_cotizacion e
    ...
    case 'traer_estado':
        ...
        $sql = '
					...
					SELECT estado, estado_autorizado FROM cmx_cotizaciones_serviciocliente cab
    ...
    case 'traer_costo':
        ...
        $sql = "SELECT costo FROM cmx_para_tipo_sevicio
    ...
    case 'traer_tipo_mercancia':
        ...
        $sql = 'SELECT b.nombre, a.codigo, a.partida,

					b.id

						FROM cmx_rndc_codificacion_producto a

						INNER JOIN cmx_para_tipo_mercancia b

						ON a.id=b.id_producto_mn
    ...
    case 'update_cotizacion':
        ...
        //while($b = count($contador)){

        $tarifa = $merca->valor_tarifa[$b];

        $flete = $merca->valor_flete[$b];

        $util = $merca->valor_utilidad[$b];

        $renta = $merca->valor_rentabilidad[$b];

        $bloque = $merca->id_bloque[$b];

        $sql2 = "UPDATE cmx_detalle_mercancia2 SET
        ...
        $sql3 = "UPDATE cmx_estados_cotizacion

        SET estado='F1',

        estado_autorizado='por autorizar'

        WHERE n_cotizacion=" . $cotizacion;

        $result3 = $Data->ejecuteRegistro($sql3);

        $sql4 = "UPDATE cmx_cotizaciones_serviciocliente

        set estado_autorizado='por autorizar',
        ...
        $sql3 = "UPDATE cmx_cotizaciones_serviciocliente

        SET estado_autorizado='autorizado',
    ...
    case 'insert_estado_cotizacion':
        ...
        $sql = '
					...
					INSERT INTO cmx_estados_cotizacion
						...
						(id,n_cotizacion,estado,estado_autorizado,hora,fecha,user_log)
    ...
    case 'solicitud_vehiculo':
        ...
        if ($sql) {
            ...
            for ($i = 0; $i < count($grupo); $i++) {
                ...
                $sqlg = "INSERT INTO cmx_grupocliente_servicio

								(id,id_grupo,id_servicio)

								VALUES(null," . $gseleccionado . "," . $id_servicio . ")";

                $result2 = $Data->ejecuteRegistro($sqlg);
            }
            ...
            for ($m = 0; $m < count($horacliente); $m++) {
                ...
                $result2 = $Data->ejecuteRegistro($sqlm);
            }
        }
    ...
    case 'consulte_aprobacion':
        ...
        $sql = 'SELECT  a.id, b.id AS parejaod, a.*, b.tipo_servicio_mer, b.origen,
					...
					INNER JOIN cmx_solicitud_vehiculo2 AS soli
        ...
        // $origen=$result["rowsData"][0]["origen"];

        // $destino=$result["rowsData"][0]["destino"];

        // $sqlo="SELECT CONCAT(municipio,'-',depto) as origi FROM cmx_municipios

        //         WHERE rndc_codigo_ciudad=".$origen." ";

        // $resulto = $Data->getConsulta($sqlo);

        // $sqld="SELECT CONCAT(municipio,'-',depto) as desti FROM cmx_municipios

        //     WHERE rndc_codigo_ciudad=".$destino." ";

        // $resultd = $Data->getConsulta($sqld);

        $return["result"] = $result["rowsData"];

        // $return["resulto"] = $resulto["rowsData"];

        // $return["resultd"] = $resultd["rowsData"];
    ...
    case 'agregar_select_editar':
        ...
        // $sqlmuni='SELECT * FROM cmx_municipios';

        // $resultmun= $Data->getConsulta($sqlmuni);

        // $tmpSelector = Array();

        // $arraymuni = Array();

        // foreach($result["rowsData"] as $index => $element){

        //     $tmpSelector[$index]['nombre']  = $element['nombre'];

        // }

        // $result["rowsData"][0]['nombre'] = $tmpSelector;

        // foreach($resultmun["rowsData"] as $index => $element){

        //     $tmpSelector[$index]['municipio']  = $element['municipio'];

        // }

        // $result["rowsData"][0]['municipio'] = $tmpSelector;
    ...
    case 'consultem':
        ...
        $sql3 = '
					...
					SELECT nombre FROM cmx_para_tipo_vehiculo
    ...
    case 'consulteE':
        ...
        $sql = '
					...
					SELECT * FROM
					 ...
					 cmx_detalle_servespecial2  espe
    ...
    case 'cabecera_no_editar':
        ...
        //     SELECT * FROM cmx_estado_cotizacion WHERE naturaleza="Activo"

        // ';

        // $resultestado = $Data->getConsulta($sql2);

        // $array = Array();

        // foreach($resultestado["rowsData"] as $index => $element){

        //     if(strcasecmp($element['nombre'], $result["rowsData"][$index1]['estado']==0)){
        ...
        //     $array[$index]['elestado'] = $element['nombre'];

        // }

        // $result["rowsData"][$index1]['estado'] = $array;
    ...
    case 'ver_mer':
        ...
        SELECT cdm.*, tv.nombre FROM

        cmx_detalle_mercancia2 cdm
    ...
    case 'solicitud_servicio':
        ...
        /* $sql = "SELECT s.*, a.nombre,pe.fecha_estimada_entrega AS fecha_cargue,pe.hora_estimada AS hora_cargue FROM cmx_solicitud_vehiculo2  s
				INNER JOIN cmx_agencias a ON s.agencia=a.id 
                INNER JOIN cmx_ruta_puntosentrega pe ON s.id=pe.cod_ini_ruta
                WHERE s.id=" . $soli_servi . "    ";*/
        ...
        rm.nombre AS remitente, des.nombre AS destinatario, ds.fecha_estimada_entrega AS fecha_descargue,
         ds.hora_estimada AS hora_descargue, pe.id_punto AS punto_rem, ds.id_punto AS punto_des
        FROM cmx_solicitud_vehiculo2  s
        INNER JOIN cmx_agencias a ON s.agencia=a.id 
        INNER JOIN cmx_ruta_puntosentrega pe ON s.id=pe.cod_ini_ruta
    ...
    case 'traer_valores_totales':
		...
		SELECT total_transporte, total_especial, total_cotizacion, tmer_flete, tmer_utili,
    ...
    case 'requiere_cancelacion':
        ...
        $sql = "SELECT
			...
			LEFT JOIN cmx_preestudio_solicitudes_servicio pse

			ON se.id=pse.id_servicio_cliente
    ...
    case 'Cancelar_Solicitud':
        ...
        $sql = "UPDATE cmx_solicitud_vehiculo2
				...
				WHERE id=" . $numero_solicitud_servicio;
    ...
    case 'consulta_datos_subasta':
        ...
        $sql = "
			...
			ON ss.numer_solservicio=ser.id
    ...
    case 'validar_solser_subasta':
        ...
        $sql = "SELECT soli.id, es.estado, su.fecha_finaliza,
						...
						ON soli.id=sse.numer_solservicio
    ...
    case 'consulta_serviciob':
        ...
        $sql = "SELECT se.id,
			...
			WHERE se.id=" . $solicitud_servicio;
    ...
    case 'cantidad_solicitud':
        ...
        $sql = "SELECT  COUNT(soli.id) AS cantidad

		FROM cmx_solicitud_vehiculo2 AS soli
    ...
    case 'traza_solicitud':
        ...
        $idpareja = $_POST["idpareja"];
        $sql = "SELECT ss.id AS 'servicio', ps.id_solicitudpreestudio,
        ...
        ON ss.id=ps.id_servicio_cliente AND ps.es=1 AND ps.clasificacion='P'
        LEFT JOIN cmx_preestudio_solicitudes_servicio eps
        ON ss.id=eps.id_servicio_cliente AND eps.es=1 AND eps.clasificacion='E'
        LEFT JOIN cmx_subasta_solicitud_servicio sub
        ON sub.numer_solservicio=ss.id
