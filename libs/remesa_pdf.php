...
$detalle_orden = $conexion->prepare("SELECT
	ro.id_remesa,
	ro.id_orden_cargue,
	r.fecha_creacion,
	r.aplica_seguro,
	...
	rr.direccion_entrega AS remdireccion,
	cli.nombre AS remnombre,
	mn.municipio AS remcity,
	rd.direccion_entrega AS desdireccion,
	clid.nombre AS desnombre,
	mnd.municipio AS descity,
	r.valor_declarado,
	b.tipo_mercancia,
	b.naturaleza,
	b.cantidad_empaque,
	...
	b.volumen_total,
	em.empaque,
	ser.nundoc_solicitud,
	o.mer_idservicio,
	rd.observacion AS 'Observacion_Destinatario',
	ser.escenario_id,
	rt.rta_ministerio
FROM
	cmx_remesa r
	...
	-- INNER JOIN cmx_detalle_mercancia2 b ON ((ser.escenario_id IN (5,6,7,8) AND ser.n_cotizacion = b.n_cotizacion) OR (ser.escenario_id NOT IN (5,6,7,8) AND ser.idpareja_origen_destino = b.id))
...
WHERE
	ro.id_remesa = $id_rem
	AND ro.id_orden_cargue = $idord
");
$detalle_orden->execute();
$fila = $detalle_orden->fetchAll(PDO::FETCH_ASSOC);

$fecha = $fila[0]['fecha_creacion'];
$oficina = $fila[0]['agencia'];
$placa = $fila[0]['placa'];
$conductor = $fila[0]['nombre'] . ' ' . $fila[0]['apellido1'] . ' ' . $fila[0]['apellido2'];
$identifica = $fila[0]['numero_documento'];
$rnombre = $fila[0]['remnombre'];
$rciudad = $fila[0]['remcity'];
$rdireccion = $fila[0]['remdireccion'];
$dnombre = $fila[0]['desnombre'];
$dciudad = $fila[0]['descity'];
$ddireccion = $fila[0]['desdireccion'];
$idser = $fila[0]['nundoc_solicitud'];
// $mer_idservicio = $fila[0]['mer_idservicio'];
$mercancia = $fila[0]['tipo_mercancia'];

if ($fila[0]['naturaleza'] == 1) {
	$natu = 'Carga normal';
}
if ($fila[0]['naturaleza'] == 2) {
	$natu = 'Carga peligrosa';
}
if ($fila[0]['naturaleza'] == 3) {
	$natu = 'Carga extradimensionada';
}
if ($fila[0]['naturaleza'] == 4) {
	$natu = 'Carga extrapesada';
}
if ($fila[0]['naturaleza'] == 5) {
	$natu = 'Residuos Peligrosos';
}
if ($fila[0]['naturaleza'] == 6) {
	$natu = 'Semovientes';
}
if ($fila[0]['naturaleza'] == 7) {
	$natu = 'Refrigerada';
}
if ($fila[0]['naturaleza'] != 7 && $fila[0]['naturaleza'] != 6 && $fila[0]['naturaleza'] != 5 && $fila[0]['naturaleza'] != 4 && $fila[0]['naturaleza'] != 3 && $fila[0]['naturaleza'] != 2 && $fila[0]['naturaleza'] != 1) {
	$natu = $fila[0]['naturaleza'];
}
...
$obs = $fila[0]['Observacion_Destinatario'];
		...
		<tr>
			...
			<td>
				<table style="border: 0.7px solid #000000;">
					...
					<tr>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:9px;font-weight: bold;">' . $fila[0]['rta_ministerio'] . '</td>
					</tr>
				</table>
			</td>
		</tr>
	...
	<table style="border: 0.7px solid #000000;" cellpading="2">
		<thead>
			...
			<tr>
				...
				<th class="text-center" style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;width:69px;">xxxx</th>
			</tr>
			<tr>
...
foreach ($fila  as $value) {
	$tabla .= '
				<tr>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;">' . $value['mer_idservicio'] . '</td>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;">' . $value['cantidad_empaque'] . '</td>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;">' . $value['empaque'] . '</td>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;">' . $value['peso_neto_tn'] . '</td>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;">' . $value['volumen_total'] . '</td>
						<td style="font-family: Arial, Helvetica, sans-serif;font-size:7px;border: 0.7px solid #000000;text-align: center;">' . $value['tipo_mercancia'] . '</td>
					</tr>
				';
}
